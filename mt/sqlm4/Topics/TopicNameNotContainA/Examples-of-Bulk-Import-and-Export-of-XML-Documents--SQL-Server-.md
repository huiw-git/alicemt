---
title: Examples of Bulk Import and Export of XML Documents (SQL Server)
ms.custom: na
...
##  <a name="top"></a>
 You can bulk import XML documents into a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database or bulk export them from a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database. This topic provides examples of both.  
  
 To bulk import data from a data file into a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] table or non\-partitioned view, you can use the following:  
  
-   **bcp** utility  
 You can also use the **bcp** utility to export data from anywhere in a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database that a SELECT statement works, including partitioned views.  
  
-   BULK INSERT  
  
-   INSERT ... SELECT \* FROM OPENROWSET\(BULK...\)  
  
 For more information, see [Import and Export Bulk Data by Using the bcp Utility &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Import-and-Export-Bulk-Data-by-Using-the-bcp-Utility--SQL-Server-.md) and [Import Bulk Data by Using BULK INSERT or OPENROWSET&#40;BULK...&#41; &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Import-Bulk-Data-by-Using-BULK-INSERT-or-OPENROWSET-BULK...---SQL-Server-.md).  
...
#### Remarks  
 By using SINGLE\_BLOB in this case, you can avoid a mismatch between the encoding of the XML document \(as specified by the XML encoding declaration\) and the string codepage implied by the server.  
...
>  We recommended that you not enable support for Document Type Definitions \(DTDs\) if it is not required in your XML environment. Turning on DTD support increases the attackable surface area of your server, and may expose it to a denial\-of\-service attack. If you must enable DTD support, you can reduce this security risk by processing only trusted XML documents.  
 ...
 During an attempt to use a [bcp](../../Topics/TopicNameNotContainA/bcp-Utility.md) command to import XML data from a file that contains a DTD, an error similar to the following can occur:  
...
#### Sample Table  
 When you bulk import or export an XML document, you should use a [field terminator](../../Topics/TopicNameNotContainA/Specify-Field-and-Row-Terminators--SQL-Server-.md) that cannot possibly appear in any of the documents; for example, a series of four nulls \(`\0`\) followed by the letter `z`: `\0\0\0\0z`.  
...
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] does not save the XML encoding when XML data is persisted in the database. Therefore, the original encoding of XML fields is not available when XML data is exported. [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] uses UTF\-16 encoding when exporting XML data.  
