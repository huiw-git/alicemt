...
|PENDING\_FAILOVER|This state is found only on the principal server after a failover has begun, but the server has not transitioned into the mirror role.<br /><br /> When the failover is initiated, the principal database goes into the PENDING\_FAILOVER state, quickly terminates any user connections, and takes over the mirror role soon thereafter.|  
