...
# Backup and Restore: Interoperability and Coexistence (SQL Server)
  This topic describes backup\-and\-restore considerations for several features in [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)]. These features include: file restore and database startup, online restore and disabled indexes, database mirroring, and piecemeal restore and full\-text indexes.  
...
##  <a name="FileRestoreAndDbStartup"></a> File Restore and Database Startup  
 This section is relevant only for [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] databases that have multiple filegroups.  
...
>  When a database is started, only filegroups whose files were online when the database was closed are recovered and brought online.  
 ...
 ALTER DATABASE *database\_name* MODIFY FILE \(NAME **\='***filename***'**, OFFLINE\)  
...
> [!NOTE]  
>  The database mirroring feature will be removed in a future version of Microsoft [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. Avoid using this feature in new development work, and plan to modify applications that currently use this feature. Use [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] instead.  
...
> [!NOTE]  
>  To view the filegroup ID of the filegroup that contains a full\-text index, select the data\_space\_id column of [sys.fulltext\_indexes](../Topic/sys.fulltext_indexes%20\(Transact-SQL\).md).  
...
-   If the filegroup that contains the table data is restored and brought online before the filegroup that contains the full\-text index, full\-text behavior might be affected. This is because [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements that trigger a population, rebuild the catalog, or reorganize the catalog fail until the index is brought online. These statements include CREATE FULLTEXT INDEX, ALTER FULLTEXT INDEX, DROP FULLTEXT INDEX, and ALTER FULLTEXT CATALOG.  
...
##  <a name="FileBnRandCompression"></a> File Backup and Restore and Compression  
 [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] supports NTFS file system data compression of read\-only filegroups and read\-only databases.  
