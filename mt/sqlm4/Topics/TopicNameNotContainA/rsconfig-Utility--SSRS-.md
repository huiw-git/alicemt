...
|**\-s**  *servername*|Required.|Specifies the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] instance that hosts the report server database.|  
|**\-i**  *instancename*|Required if you are using named instances.|If you used a named [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] instance to host the report server database, this value specifies the named instance.|  
...
-   To configure a special account that the report server uses to log on to a remote database server when other credentials are not available.  
 ...
 You can run the**rsconfig** utility on a local or remote instance of [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)]. You cannot use the **rsconfig** utility to decrypt and view values that are already set.  
...
#### Specifying a SQL Server Database User Account  
 This example shows how to configure a report server to use [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] login to connect to a remote report server database.  
...
#### Specifying a Built\-in Account  
 This example shows how to configure a report server to use a built\-in account when connecting to a local report server database. Notice that **\-u** is not used. Examples of supported built\-in account values include NT AUTHORITY\\SYSTEM for Local System and NT AUTHORITY\\NETWORKSERVICE for Network Service \([!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[winxpsvr](../../Token/Other/winxpsvr_md.md)] only\).  
