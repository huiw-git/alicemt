---
title: FileTable Compatibility with Other SQL Server Features
ms.custom: na
...
# FileTable Compatibility with Other SQL Server Features
  Describes how FileTables work with other features of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)].  
...
-   FileTable functionality is partially supported by [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)]. After a failover, FileTable data is accessible on the primary replica, but FileTable data is not accessible on readable secondary replicas.  
 ...
 **Transact\-SQL applications**  
 For TSQL applications working on the FILESTREAM \(file\_stream\) column in a FileTable, the isolation semantics are the same as with FILESTREAM datatype in a regular user table.  
...
-   When non\_transactional access is set to FULL for the database, then a transaction running under RCSI or SI has the following behavior:  
  
    -   Any [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] reads of the FileTable file\_stream column fail. INSERT and UPDATE to the column still succeed, as long as they do not read from the file\_stream column.  
  
    -   If the [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statement specifies READCOMMITTEDLOCK table hints, the reads succeed, and take locks on the rows, rather than use row versioning.  
    ...
    -   Fulltext indexing always succeeds, no matter what the database options are \(READ\_COMMITTED\_SNAPSHOT or ALLOW\_SNAPSHOT\_ISOLATION\).  
