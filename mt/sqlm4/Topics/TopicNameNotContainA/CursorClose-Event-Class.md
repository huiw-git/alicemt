...
# CursorClose Event Class
  Cursor close events occur when the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] closes and deallocates a cursor. The **CursorClose** event class describes cursor close events that occur in application programming interface \(API\) cursors. This event class occurs when a [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] cursor statement by ODBC, OLE DB, or DB\-Library is closed.  
