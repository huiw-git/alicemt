...
12. Verify that **External Columns** lists a column for each parameter in the SQL statement. The column names are **Param\_0**, **Param\_1**, and so on.  
  
     You should not change the column names. If you change the column names, [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] generates a validation error for the OLE DB Command transformation.  
...
13. If **External Columns** lists no columns you must add them manually.  
    ...
    -   Update the column names to **Param\_0**, **Param\_1**, and so on.  
