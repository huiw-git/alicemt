...
# Perform an Incremental Load of Multiple Tables
 ...
 When you perform an incremental load of multiple tables, some steps have to be performed once for all the tables, and other steps have to be repeated for each source table. You have more than one option for implementing these steps in [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)]:  
...
2.  In the control flow, use an Execute SQL task or [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] expressions to calculate the endpoints.  
