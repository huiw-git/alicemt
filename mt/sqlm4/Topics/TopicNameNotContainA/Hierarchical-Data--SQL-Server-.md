...
-   A graph of links between Web pages  
 ...
 Use [hierarchyid](../Topic/hierarchyid%20\(Transact-SQL\).md) as a data type to create tables with a hierarchical structure, or to describe the hierarchical structure of data that is stored in another location. Use the [hierarchyid functions](../Topic/hierarchyid%20Data%20Type%20Method%20Reference.md) in [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] to query and manage hierarchical data.  
...
### XML  
 An XML document is a tree, and therefore a single XML data type instance can represent a complete hierarchy. In [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] when an XML index is created, **hierarchyid** values are used internally to represent the position in the hierarchy.  
...
###  <a name="findclr"></a> Finding Ancestors by Using the CLR  
 A common operation involving two nodes in a hierarchy is to find the lowest common ancestor. This can be written in either [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] or CLR, because the **hierarchyid** type is available in both. CLR is recommended because performance will be faster.  
...
```  
 ...
 To use the **ListAncestor** and **CommonAncestor** methods in the following [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] examples, build the DLL and create the **HierarchyId\_Operations** assembly in [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] by executing code similar to the following:  
...
###  <a name="lowestcommon"></a> Finding the Lowest Common Ancestor  
 Using the **HierarchyId\_Operations** class defined above, create the following [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] function to find the lowest common ancestor involving two nodes in a hierarchy:  
