...
-   If you use the backup medium before the set number of days has passed, [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] issues a warning message. [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] does not issue a warning unless you change the default.  
...
3.  Copy and paste the following example into the query window and click **Execute**. This example shows how to use [sp\_configure](../Topic/sp_configure%20\(Transact-SQL\).md) to set the value of the `media retention` option to `60` days.  
