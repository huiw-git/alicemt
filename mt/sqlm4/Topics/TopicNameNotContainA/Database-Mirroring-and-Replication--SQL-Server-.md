...
-   Metadata and objects that exist outside the database are not copied to the mirror, including logins, jobs, linked servers, and so on. If you require the metadata and objects at the mirror, you must copy them manually. For more information, see [Management of Logins and Jobs After Role Switching &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Management-of-Logins-and-Jobs-After-Role-Switching--SQL-Server-.md).  
...
1.  Configure the Publisher:  
    ...
    1.  We recommend using a remote Distributor. For more information about configuring distribution, see [Configure Distribution](../../Topics/TopicNameNotContainA/Configure-Distribution.md).  
  
    2.  You can enable a database for snapshot and transactional publications and\/or merge publications. For mirrored databases that will contain more than one type of publication, you must enable the database for both types at the same node using [sp\_replicationdboption](../Topic/sp_replicationdboption%20\(Transact-SQL\).md). For example, you could execute the following stored procedure calls at the principal:  
        ...
        ```  
         ...
         For more information about creating publications, see [Publish Data and Database Objects](../../Topics/TopicNameNotContainA/Publish-Data-and-Database-Objects.md).  
  
2.  Configure database mirroring. For more information, see [Establish a Database Mirroring Session Using Windows Authentication &#40;SQL Server Management Studio&#41;](../../Topics/TopicNameContainA/Establish-a-Database-Mirroring-Session-Using-Windows-Authentication--SQL-Server-Management-Studio-.md) and [Setting Up Database Mirroring &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Setting-Up-Database-Mirroring--SQL-Server-.md).  
  
3.  Configure distribution for the mirror. Specify the mirror name as the Publisher, and specify the same Distributor and snapshot folder that the principal uses. For example, if you are configuring replication with stored procedures, execute [sp\_adddistpublisher](../Topic/sp_adddistpublisher%20\(Transact-SQL\).md) at the Distributor; and then execute [sp\_adddistributor](../Topic/sp_adddistributor%20\(Transact-SQL\).md) at the mirror. For **sp\_adddistpublisher**:  
  
    -   Set the value of the **@publisher** parameter to the network name of the mirror.  
  
    -   Set the value of the **@working\_directory** parameter to the snapshot folder used by the principal.  
...
4.  Specify the mirror name for the **–PublisherFailoverPartner** agent parameter. Agent This parameter is required for the following agents to identify the mirror after failover:  
    ...
    -   [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] replication listener \(replisapi.dll: for merge subscriptions synchronized using Web synchronization\)  
...
5.  Add the principal and mirror to Replication Monitor. For more information, see [Add and Remove Publishers from Replication Monitor](../../Topics/TopicNameNotContainA/Add-and-Remove-Publishers-from-Replication-Monitor.md).  
...
-   Administration and monitoring must occur at the active server. In [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], publications appear under the **Local Publications** folder only for the active server. For example, if you failover to the mirror, the publications are displayed at the mirror and are no longer displayed at the principal. If the database fails over to the mirror, you might need to manually refresh [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] and Replication Monitor for the change to be reflected.  
...
-   When using stored procedures or Replication Management Objects \(RMO\) to administer replication at the mirror, for cases in which you specify the Publisher name, you must specify the name of the instance on which the database was enabled for replication. To determine the appropriate name, use the function [publishingservername](../Topic/PUBLISHINGSERVERNAME%20\(Transact-SQL\).md).  
    ...
    > [!NOTE]  
    >  In some cases, such as **sp\_addpublication**, the **@publisher** parameter is supported only for non\-[!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Publishers; in these cases, it is not relevant for [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database mirroring.  
  
-   To synchronize a subscription in [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] after a failover: synchronize pull subscriptions from the Subscriber; and synchronize push subscriptions from the active Publisher.  
...
-   If database mirroring is removed completely, the mirror database is in a recovery state and must be restored in order to become functional. The behavior of the recovered database with respect to replication depends on whether the KEEP\_REPLICATION option is specified. This option forces the restore operation to preserve replication settings when restoring a published database to a server other than that on which the backup was created. Use the KEEP\_REPLICATION option only when the other publication database is unavailable. The option is not supported if the other publication database is still intact and replicating. For more information about KEEP\_REPLICATION, see [RESTORE &#40;Transact-SQL&#41;](../Topic/RESTORE%20\(Transact-SQL\).md).  
