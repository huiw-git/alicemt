...
# Broker:Corrupted Message Event Class
  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] creates a **Broker:Corrupted Message** event when Service Broker receives a corrupted message.  
...
|**Host Name**|**nvarchar**|The name of the computer on which the client is running. This data column is populated if the host name is provided by the client. To determine the host name, use the HOST\_NAME function.|8|Yes|  
...
|**Severity**|**int**|If an error caused [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] to drop the message, the severity of the error.|29|No|  
