...
# Configure a Report Server for Remote Administration
  In [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)], you can configure report server instances locally or remotely. To configure a remote report server instance, you can use the Reporting Services Configuration tool or write custom code that uses the [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] Windows Management Instrumentation \(WMI\) provider. The Reporting Services Configuration tool provides a graphical interface to the WMI provider so that you can configure a report server without having to write code. When you start the tool, you can specify a remote server to connect to.  
...
-   Enable remote WMI requests to pass through the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Windows firewall.  
...
1.  Click **Start**, point to **Programs**, point to [!INCLUDE[ssCurrentUI](../../Token/Other/ssCurrentUI_md.md)], point to **Configuration Tools**, and click **SQL Server Configuration Manager**.  
  
2.  In the left pane, expand **SQL Server Network Configuration**, and then click **Protocols** for the instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)].  
  
3.  In the details pane, enable the TCP\/IP and Named Pipes protocols, and then restart the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service.  
