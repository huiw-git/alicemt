...
3.  Issue the following statement on the principal server:  
     ...
     [ALTER DATABASE](../Topic/ALTER%20DATABASE%20Database%20Mirroring%20\(Transact-SQL\).md) *database\_name* SET PARTNER FAILOVER, where *database\_name* is the mirrored database.  
...
> [!NOTE]  
>  Transactions that have been prepared by using the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Distributed Transaction Coordinator but are still not committed when a failover occurs are considered aborted after the database has failed over.  
