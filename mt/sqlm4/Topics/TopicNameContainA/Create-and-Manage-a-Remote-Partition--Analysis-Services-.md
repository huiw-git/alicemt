...
# Create and Manage a Remote Partition (Analysis Services)
  When partitioning a measure group, you can configure a secondary database on a remote [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance as partition storage.  
  
 Remote partitions for a cube \(called the master database\) are stored in a dedicated [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database on the remote instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] \(called the secondary database\).  
  
 A dedicated secondary database can store remote partitions for one and only one master database, but the master database can use multiple secondary databases, as long as all the secondary databases are on the same remote instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)]. Dimensions in a database dedicated to remote partitions are created as linked dimensions.  
  
## Prerequisites  
 Before you create a remote partition, the following conditions must be met:  
  
-   You must have a second [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance and dedicated database to store the partitions. The secondary database is single\-purpose; it provides storage of remote partitions for a master database.  
  
-   Both server instances must be the same version. Both databases should be the same functional level.  
  
-   Both instances must be configured for TCP connections. [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] does not support creation of remote partitions by using the HTTP protocol.  
...
-   The service account for the instance running the master database must have administrative access to the remote instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)]. If the service account changes, you must update permissions on both the server and database.  
  
-   You must be an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] administrator on both computers.  
...
## Configure remote partitions  
 Two separate computers that are running an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] are each required to create a remote partition arrangement that designates one computer as the master server and the other computer as the subordinate server.  
 ...
 You will use both [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] and [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)] to complete this procedure.  
...
1.  On the subordinate server: In [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], right\-click your connected [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance in Object Explorer and select **Properties**. Set both **Feature\\LinkToOtherInstanceEnabled** and **Feature\\LinkFromOtherInstanceEnabled** to **True**.  
...
3.  On the master server: In [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], right\-click your connected [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance in Object Explorer and select **Properties**. Set both **Feature\\LinkToOtherInstanceEnabled** and **Feature\\LinkFromOtherInstanceEnabled** to **True**.  
...
## Administering remote partitions  
 [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] supports both parallel and sequential processing of remote partitions. The master database, where the partitions were defined, coordinates the transactions among all the instances that participate in processing the partitions of a cube. Processing reports are then sent to all instances that processed a partition.  
  
 A cube that contains remote partitions can be administered together with its partitions on a single instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)]. However, the metadata for the remote partition can be viewed and updated only on the instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] where the partition and its parent cube were defined. The remote partition cannot be viewed or updated on the remote instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)].  
