---
title: Command Events Event Category
H1: na
ms.custom: na
ms.prod: sql-server-2016
ms.reviewer: na
ms.suite: na
ms.technology: 
  - analysis-services
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 23140c9f-3f5b-4755-920e-1246cbf0d1a3
---
# Command Events Event Category
  The Command Events event category has the event classes described in the following table.  
  
|Event Class|Event Id|Description|  
|-----------------|--------------|-----------------|  
|Command Begin|15|Collects all command begin events since the trace started.|  
|Command End|16|Collects all command end events since the trace started.|  
  
 For information about the columns associated with each of the Command Events event classes, see [Command Events Data Columns](../../Topics/TopicNameNotContainA/Command-Events-Data-Columns.md).  
  
## See Also  
 [Analysis Services Trace Events](../../Topics/TopicNameNotContainA/Analysis-Services-Trace-Events.md)  
  
  