...
*Hash* indexes are discussed in more detail in a [closely related article](Hash%20Indexes%20for%20Memory%2dOptimized%20Tables.xml).  
  
  
*Columnstore* indexes are discussed in [another article](../../Topics/TopicNameNotContainA/Columnstore-Indexes-Guide.md).  
...
To be declared with DURABILITY = SCHEMA_AND_DATA, the memory-optimized table must have a primary key. The PRIMARY KEY NONCLUSTERED clause in the following CREATE TABLE statement satisfies two requirements:  
...
- Provides the primary key that is required for the SCHEMA_AND_DATA clause.  
    ...
    (  
        SupportEventId   int     not null    identity,  
...
### C.2 Strengths of hash indexes  
  
  
A [hash index](Hash%20Indexes%20for%20Memory%2dOptimized%20Tables.xml) is preferable over a nonclustered index when:  
  
- Queries test the indexed column by use of a WHERE clause with an exact equality, as in the following:  
...
### C.3 Summary table to compare index strengths  
...
| Index seek on equality predicates (=). | Yes <br/> (Full key is required.) | Yes <br/> (Note-1) | Yes |  
| Index seek on inequality and range predicates <br/> (>, <, <=, >=, BETWEEN). | No <br/> (Results in an index scan.) | Yes <br/> (Note-1) | Yes |  
...
*Note-1:* For a nonclustered memory-optimized index, the full key is not required to perform an efficient index seek. However, an inefficient scan will occur if the only tested column is not the first column in the index key.  
