...
The topic lists the system catalog views and DMVs that are related to [!INCLUDE[rsql_productname_md](../../Topics/TopicNameContainA/includes/rsql_productname_md.md)]. 
...
> [!NOTE]
> 
> System views related to [!INCLUDE[rsql_productname_md](../../Topics/TopicNameContainA/includes/rsql_productname_md.md)] are not present unless you have enabled the feature.

## System Configuration and System Resources

You can monitor and analyze the resources used by R scripts by using [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] system catalog views and DMVs.
...
+ [sys.dm_os_performance_counters (Transact-SQL)](sys.dm_os_performance_counters%20\(Transact-SQL\).md)
  ...
  + **Parallel Executions**: Number of times that a script included the @parallel specification and that [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] was able to generate and use a parallel query plan
  ...
  + **Implied Auth. Logins**: Number of times that an ODBC loopback call was made using implied authentication; that is, the [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] executed the call on behalf of the user sending the script request
...
+ sys.dm_external_script_requests
  ...
  + **external_script_request_id**: The process ID as received by [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)]  
  + **language**: The keyword of a supported script language. Currently only `R` is supported.
  + **degree_of_parallelism**:  An integer indicating the number of parallel processes that were created. This might be different from the number of parallel processes that were requested. 
...
+ sys.dm_external_script_execution_stats (Transact-SQL)
  ...
  This DMV is provided for internal monitoring (telemetry) to track how many R calls are made on an instance. The telemetry service starts when [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] does and increments a disk-based counter each time a specific R function is called.
 ...
 For more information about system performance counters used by [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)], see [Use SQL Server Objects](../../Topics/TopicNameNotContainA/Use-SQL-Server-Objects.md).
...
+ [sys.resource_governor_external_resource_pools](sys.resource_governor_external_resource_pools%20\(Transact-SQL\).md)
  ...
  In Enterprise Edition, you can configure additional external resource pools: for example, you might decide to handle resources for R jobs running in [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] separately from those that originate from a remote client. 
...
+ [sys.dm_resource_governor_external_resource_pool_affinity](sys.dm_resource_governor_external_resource_pool_affinity%20\(Transact-SQL\).md)
  ...
  Returns one row per scheduler in [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] where each scheduler is mapped to an individual processor. Use this view to monitor the condition of a scheduler or to identify runaway tasks.
  ...
  The affinity schedule maps the resource pool to the [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] schedules identified by the given IDs. These IDs map to the values in the scheduler_id column in sys.dm_os_schedulers (Transact-SQL).
...
For general information about monitoring [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] instances, see [Catalog Views](Catalog%20Views%20\(Transact-SQL\).md) and [Resource Governor Related Dynamic Management Views](Resource%20Governor%20Related%20Dynamic%20Management%20Views%20\(Transact-SQL\).md).

## R Script Execution and Monitoring

R scripts that run in [!INCLUDE[ssNoVersion_md](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] are started by the [!INCLUDE[rsql_launchpad_md](../../Topics/TopicNameNotContainA/includes/rsql_launchpad_md.md)] interface. However, the Launchpad is not resource governed or monitored separately, as it is assumed to be a secure service provided by Microsoft that manages resources appropriately.
...
A job object allows groups of processes to be managed as a unit. Each job object is hierarchical and controls the attributes of all processes associated with it. Operations performed on a job object affect all processes associated with the job object. 
