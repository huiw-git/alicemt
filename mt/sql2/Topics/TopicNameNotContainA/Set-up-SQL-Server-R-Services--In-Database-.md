...
1. Open [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/includes/ssManStudioFull_md.md)]. If it is not already installed, you can rerun the SQL Server setup wizard to open a download link and install it.  
...
10. Restart the SQL Server service for the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] instance. This will automatically restart the related [!INCLUDE[rsql_launchpad](../../Topics/TopicNameNotContainA/includes/rsql_launchpad_md.md)] service as well. You can restart the service using the Services panel in Control Panel, or by using SQL Server Configuration Manager.  
...
10. At this point you should be able to run simple R scripts like the following in  [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/includes/ssManStudioFull_md.md)]:  
...
During setup, 20 new Windows user accounts are created for the purpose of running tasks under the security token of the [!INCLUDE[rsql_launchpad_md](../../Topics/TopicNameNotContainA/includes/rsql_launchpad_md.md)] service. You can view these accounts in the Windows user group, **SQLRUserGroup**.    
...
1. In [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/includes/ssManStudioFull_md.md)], in Object Explorer, expand **Security**, right-click **Logins**, and select **New Login**.  
...
-   **Modify the number of worker accounts used by [!INCLUDE[rsql_launchpad](../../Topics/TopicNameNotContainA/includes/rsql_launchpad_md.md)]**, their names, and passwords  
...
-   **Give your R users or logins read, write, or DDL permissions as needed in additional databases**  
     ...
     While running R scripts, the user account may need to read data from other databases, create new tables to store results, and write data into tables. Therefore, for each user who will be executing R scripts, be sure that the user account has **db_datareader**, **db_datawriter**, or **db_ddladmin** permissions on the specific database.   
       
     For example, the following [!INCLUDE[tsql](../../Topics/TopicNameContainA/includes/tsql_md.md)] statement gives the SQL login *MySQLLogin* the rights to run SQL queries in the *RSamples* database. To run this statement, the SQL login must already exist in the security context of the server.  
