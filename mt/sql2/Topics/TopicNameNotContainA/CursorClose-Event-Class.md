...
# CursorClose Event Class
  Cursor close events occur when the [!INCLUDE[ssDE](../../Topics/TopicNameContainA/includes/ssDE_md.md)] closes and deallocates a cursor. The **CursorClose** event class describes cursor close events that occur in application programming interface (API) cursors. This event class occurs when a [!INCLUDE[tsql](../../Topics/TopicNameContainA/includes/tsql_md.md)] cursor statement by ODBC, OLE DB, or DB-Library is closed.  
