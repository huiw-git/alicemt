...
# Full File Backups (SQL Server)
  This topic is relevant for [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] databases that contain multiple files or filegroups.  
  
 The files in a [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] database can be backed up and restored individually. Also, you can specify a whole filegroup instead of specifying each constituent file individually. Note that if any file in a filegroup is offline (for example, because the file is being restored), the whole filegroup is offline and cannot be backed up.  
...
### File Backups and the Full Recovery Model  
 ...
 ![Strategy combining database, file, and log backups](../../Topics/TopicNameNotContainA/media/BnR_RMfull_3_FullDb_Filegrps_Log_Backups.gif "BnR_RMfull_3_FullDb_Filegrps_Log_Backups")  
