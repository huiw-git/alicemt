...
---
# Locals Window
  The **Locals** window displays information about the local expressions in the current scope of the [!INCLUDE[tsql](../../Topics/TopicNameContainA/includes/tsql_md.md)] debugger. The scope is set to the current call stack frame that is selected in the **Call Stack** window. You must be in debug mode to display the local expressions.  
...
## Columns  
 **Name**  
 Is the name of the local expression. The [!INCLUDE[tsql](../../Topics/TopicNameContainA/includes/tsql_md.md)] debugger lists variables, parameters, and the system functions that have names that start with @@.  
