---
title: Select rows to migrate by using a filter predicate (Stretch Database)
H1: na
ms.custom: 
  - SQL2016_New_Updated
...
-   The columns used by the function are schema bound. You can't alter these columns as long as a table is using the function as its filter predicate.  
 ...
 When you assign a filter function to a table, specify the column names passed to the filter function with a one-part name. If you specify a three-part name when you pass the column names, subsequent queries will fail.

For example, subsequent queries will fail if you specify a three-part column name as shown in the following example.

```tsql
ALTER TABLE Employees SET ( REMOTE_DATA_ARCHIVE = ON ( FILTER_PREDICATE=dbo.fn_stretchpredicate(dbo.Employees.DateJoined) , MIGRATION_STATE = OUTBOUND ))
```

Instead, specify the filter function with a one-part column name as shown in the following example.

```tsql
ALTER TABLE Employees SET ( REMOTE_DATA_ARCHIVE = ON  ( FILTER_PREDICATE=dbo.fn_stretchpredicate(DateJoined) , MIGRATION_STATE = OUTBOUND ))
```
...
2. Wait for migration to finish. You can check the status in **Stretch Database Monitor** from SQL Server Management Studio, or you can query the **sys.dm_db_rda_migration_status** view. For more info, see Monitor and troubleshoot data migration or sys.dm_db_rda_migration_status.  
...
## Check the filter predicate applied to a table  
 To check the filter predicate applied to a table, open the catalog view **sys.remote\_data\_archive\_tables** and check the value of the **filter\_predicate** column. If the value is null, the entire table is eligible for archiving. For more info, see [sys.remote_data_archive_tables &#40;Transact-SQL&#41;](../Topic/sys.remote_data_archive_tables%20\(Transact-SQL\).md).  
  
## Security concerns for filter predicates  
A compromised account with db_owner privileges can do the following things.  
  
-   Initiate a denial of service against Stretch\-enabled tables by creating a table-valued function that consumes large amounts of server resources or waits for an extended period.  
  
-   Infer the content of a table to which the user does not otherwise have access by carefully defining a table-valued function that reveals information about rows eligible for migration.  
