---
title: Determining if a Table or Stored Procedure Should Be Ported to In-Memory OLTP
H1: na
ms.custom: 
  - SQL2016_New_Updated
...
# Determining if a Table or Stored Procedure Should Be Ported to In-Memory OLTP
  The Transaction Performance Analysis report in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] helps you evaluate if In\-Memory OLTP will improve your database application’s performance. The report also indicates how much work you must do to enable In\-Memory OLTP in your application. After you identify a disk\-based table to port to In\-Memory OLTP, you can use the [Memory Optimization Advisor](../../Topics/TopicNameNotContainA/Memory-Optimization-Advisor.md), to help you migrate the table. Similarly, the [Native Compilation Advisor](../../Topics/TopicNameNotContainA/Native-Compilation-Advisor.md) will help you port a stored procedure to a natively compiled stored procedure. For information about migration methodologies, see [In\-Memory OLTP – Common Workload Patterns and Migration Considerations](https://msdn.microsoft.com/library/dn673538.aspx).  
...
-   Help you plan and execute your migration to In\-Memory OLTP. The migration path from a disk based table to a memory\-optimized table can be time consuming. The Memory\-Optimization Advisor helps you identify the incompatibilities in your table that you must remove before moving the table to In\-Memory OLTP. The Memory\-Optimization Advisor also helps you understand the impact that the migration of a table to a memory\-optimized table will have on your application.  
 ...
 The Transaction Performance Analysis report and the migration advisors are installed as part of SQL Server Management Studio \(SSMS\) when you select **Management Tools—Basic** or **Management Tools—Advanced** when you install [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)], or when you [Download SQL Server Management Studio](https://msdn.microsoft.com/library/mt238290.aspx).  
...
-   Migration Difficulties Section  
 ...
 Scan and contention statistics on the table details report is gathered and aggregated from sys.dm\_db\_index\_operational\_stats \(Transact\-SQL\).  
...
-   Table References Section  
 ...
 Execution Statistics on the stored procedure details report is gathered and aggregated from sys.dm\_exec\_procedure\_stats \(Transact\-SQL\). The references are obtained from sys.sql\_expression\_dependencies \(Transact\-SQL\).  
...
## Generating In\-Memory OLTP Migration Checklists  
 ...
 You can generate a migration checklist in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] by using the **Generate In\-Memory OLTP Migration Checklists** command or by using PowerShell.  
...
3.  Verify the following.  
    ...
    -   The migration checklist report is generated for all tables and stored procedures in the database, and the report is in the location specified by folder\_path.  
...
3.  Verify the following.  
    ...
    -   A migration checklist report is generated for all tables and stored procedures in the database, and the report is in the location specified by folder\_path.  
  
    -   A migration checklist report for \<object\_name\> is the only report in the location specified by folder\_path2.  
