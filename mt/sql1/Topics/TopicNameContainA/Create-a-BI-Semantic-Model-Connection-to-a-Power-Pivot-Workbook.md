...
# Create a BI Semantic Model Connection to a Power Pivot Workbook
  Use the information in this topic to set up a BI semantic model connection that redirects to a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook in the same farm.  
  
 After you create a BI semantic model connection and configure SharePoint permissions, you can use it as a data source for Excel or [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)] reports.  
...
##  <a name="bkmk_prereq"></a> Review Prerequisites  
 ...
 You must know the URL of the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook for which you are setting up a BI semantic model connection \(for example, http:\/\/adventure\-works\/shared documents\/myworkbook.xlsx\). The workbook must be in the same farm.  
...
2.  Set the **Server** property to the SharePoint URL of the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook \(for example, **http:\/\/mysharepoint\/shared documents\/myWorkbook.xlsx**. In a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for SharePoint deployment, data can be loaded on any server in the farm. For this reason, data source connections to [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data specify just the path to the workbook. The [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] System Service determines which server loads the data.  
  
     Do not use the **Database** property; it is not used when specifying the location of a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook.  
  
     Your page should look similar to the following illustration.  
  
     ![BISM connection page showing URL to workbook](../../Images/Image/ImageNotContaina/SSAS_BISMConnection_PPVTDS.gif "SSAS_BISMConnection_PPVTDS")  
...
##  <a name="bkmk_userdb"></a> Configure SharePoint Permissions on the Workbook  
 If you are using a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] database inside an Excel workbook, the SharePoint permissions on the Excel workbook determine data access via the BI semantic model connection. All users who access the workbook must have Read permissions on the workbook in order to use it as an external data source.  
