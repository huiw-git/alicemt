...
# Back Up a Database Master Key
  This topic describes how to back up a database master key in [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] by using [!INCLUDE[tsql](../../Token/Other/tsql_md.md)]. The database master key is used to encrypt other keys and certificates inside a database. If it is deleted or corrupted, [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] may be unable to decrypt those keys, and the data encrypted using them will be effectively lost. For this reason, you should back up the database master key and store the backup in a secure off\-site location.  
...
1.  In [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], connect to the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] instance containing the database master key you wish to back up.  
