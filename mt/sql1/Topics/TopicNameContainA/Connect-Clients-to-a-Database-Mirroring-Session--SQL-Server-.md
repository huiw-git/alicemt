...
To connect to a database mirroring session, a client can use either [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Native Client or .NET Framework Data Provider for [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)]. When configured for a [!INCLUDE[ssCurrent](../../Topics/TopicNameContainA/tokens/ssCurrent_md.md)] database, these data access providers both fully support database mirroring. For information about programming considerations for using a mirrored database, see [Using Database Mirroring](assetId:///71b15712-7972-4465-9274-e0ddc271eedc). In addition, the current principal server instance must be available and the login of the client must have been created on the server instance. For more information, see [Troubleshoot Orphaned Users (SQL Server)](../../Topics/TopicNameNotContainA/Troubleshoot-Orphaned-Users--SQL-Server-.md). Client connections to a database mirroring session do not involve the witness server instance, if one exists.  
...
>  If the mirroring session is paused, the client typically connects to the principal server and the downloads the partner name. However, the database is unavailable to the client until mirroring resumes.  
 ...
 ![Client connection if initial partner is principal](../../Topics/TopicNameContainA/images/DBM_initial_connection.gif "DBM_initial_connection")  
...
|4|3.6 **+(**0.08 **\*** 15**)**|4.8 seconds|  
 ...
 ![Maximum retry delays for 15 second login timeout](../../Topics/TopicNameContainA/images/DbM_Retry_Algorithm.gif "DbM_Retry_Algorithm")  
...
>  If the server instance is stopped, then the connection request fails immediately.  
 ...
 ![Retry&#45;delay algorithm](../../Topics/TopicNameContainA/images/DbM_Retry_Delay_Algorithm.gif "DbM_Retry_Delay_Algorithm")  
...
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Native Client verifies that it connects to a principal server instance but not whether this instance is the partner of server instance specified in the initial partner name of the connection string.  
...
### Impact of Redirection on a Client Application  
 ...
 After being redirected to the failover partner, a client can experience unexpected results when using a [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] USE statement to use a different database. This can happen if the current principal server instance (the failover partner) has a different set of databases than the original principal server (the initial partner).  
