...
# Create a Cross-Domain Rule
  This topic describes how to create a cross\-domain rule for a composite domain in a knowledge base in [!INCLUDE[ssDQSnoversion](../../Token/Other/ssDQSnoversion_md.md)] \(DQS\). A cross\-domain rule tests the relationship between values in single domains that are included in a composite domain. A cross\-domain rule must hold true across a composite domain in order for domain values to be considered accurate and conformant to business requirements. A cross\-domain rule is used to validate, correct, and standardize domain values.  
...
####  <a name="Permissions"></a> Permissions  
 You must have the dqs\_kb\_editor or the dqs\_administrator role on the DQS\_MAIN database to create a cross\-domain rule.  
