...
>  For information on SQL Server backup to the Windows Azure Blob storage service, see, [SQL Server Backup and Restore with Microsoft Azure Blob Storage Service](../../Topics/TopicNameNotContainA/SQL-Server-Backup-and-Restore-with-Microsoft-Azure-Blob-Storage-Service.md) and [SQL Server Backup to URL](../../Topics/TopicNameNotContainA/SQL-Server-Backup-to-URL.md).  
...
>  When you specify a back up task by using [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/includes/ssManStudioFull_md.md)], you can generate the corresponding [!INCLUDE[tsql](../../Topics/TopicNameContainA/includes/tsql_md.md)] [BACKUP](../Topic/BACKUP%20\(Transact-SQL\).md) script by clicking the **Script** button and selecting a script destination.  
...
### Example
#### Striped Backup to URL
The following examples performs a full database backup of `Sales` to the Microsoft Azure Blob storage service.  The storage Account name is `mystorageaccount`.  The container is called `myfirstcontainer`.  A stored access policy has been created with read, write, and list, rights.  The SQL Server credential, `https://mystorageaccount.blob.core.windows.net/myfirstcontainer`, was created using a shared access signature that is associated with the stored access policy.
...
4.  Click **Add** and the **Create Backup File** dialog box will open.

5.  Select credential, `https://mystorageaccount.blob.core.windows.net/myfirstcontainer`, from the **SQL Credential:** drop-down list.
