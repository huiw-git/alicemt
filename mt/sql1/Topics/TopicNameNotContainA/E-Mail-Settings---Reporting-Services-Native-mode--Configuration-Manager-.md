...
The following example illustrates the settings in the RSreportserver.config file for a remote SMTP server. To read about the setting descriptions and valid values, see [Rsreportserver.config Configuration File](../../Topics/TopicNameNotContainA/RsReportServer.config-Configuration-File.md) in SQL Server Books Online.
...
> [!NOTE] It is recommended that you configure the mail server through the Reporting Services Configuration Manager.
