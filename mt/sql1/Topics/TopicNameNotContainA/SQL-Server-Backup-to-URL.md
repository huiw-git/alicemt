...
> [!NOTE]  
>  To create a [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] file-snapshot backup, or overwrite an existing media set, you must use Transact-SQL, Powershell or C# rather than the Back Up task in SQL Server Management Studio.  
...
3.  On the **General** page in the **Destination** section the **URL** option is available in the **Back up to:** drop down box.  The **URL** option is used to create a backup to Windows Azure storage. Click **Add** and the **Create Backup File** dialog box will open:
   
    1.  **SQL Credential:** Select an existing SQL Server Credential created from a Shared Access Signature from the drop-down list.  
  
    2.  **Azure storage container:** The name of the Windows Azure storage container to store the backup files.  
