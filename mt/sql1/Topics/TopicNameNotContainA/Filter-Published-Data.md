...
## Static Row Filters  
 ...
 ![Row filtering](../../Topics/TopicNameNotContainA/images/repl_16.gif "repl_16")  
...
## Column Filters  
 ...
 ![Column filtering](../../Topics/TopicNameNotContainA/images/repl_17.gif "repl_17")  
  
 You can also use row and column filtering together, as illustrated here.  
  
 ![Row and column filtering](../../Topics/TopicNameNotContainA/images/repl_18.gif "repl_18")  
...
-   Transactional replication allows you to replicate an indexed view as a view or as a table. If you replicate the view as a table, you cannot filter columns from the table.  
 ...
 Row filters are not designed to work across databases. [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] intentionally restricts the execution of **sp_replcmds** (which filters execute under) to the database owner (**dbo**). The **dbo** does not have cross database privileges. With the addition of CDC (Change Data Capture) in [!INCLUDE[ssKatmai](../../Topics/TopicNameContainA/tokens/ssKatmai_md.md)] the **sp_replcmds** logic populates the change tracking tables with information that the user can return to and query. For security reasons, [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] restricts the execution of this logic so that a malicious **dbo** can’t highjack this execution path. For example, a malicious **dbo** could add triggers on CDC tables which would then get executed under the context of the user calling **sp_replcmds**, in this case the logreader agent.  If the account the agent is running under has higher privilege the malicious **dbo** could escalate his privileges.  
