...
-   Use custom formulas and functions in addition to the functions and operators that the [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] expression grammar provides. For example, validate credit card numbers that use the LUHN formula.  
...
## Writing the Script that the Component Uses  
 ...
 VSTA includes all the standard features of the [!INCLUDE[vsprvs](../../Token/Other/vsprvs_md.md)] environment, such as the color\-coded [!INCLUDE[vsprvs](../../Token/Other/vsprvs_md.md)] editor, IntelliSense, and Object Browser. The script that the Script component uses is stored in the package definition. When you are designing the package, the script code is temporarily written to a project file.  
  
 VSTA supports the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Visual C\# and [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Visual Basic programming languages.  
...
-   Select the input columns to reference.  
    ...
    > [!NOTE]  
    >  You can configure only one input when you use the [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer.  
