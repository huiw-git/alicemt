...
> [!IMPORTANT]  
>  [!INCLUDE[ssSB](../../Topics/TopicNameContainA/tokens/ssSB_md.md)] dialog security should be configured for event notifications that send messages to a service broker on a remote server. Dialog security must be configured manually according to the full security model.  
  
## Creating the Target Service  
 You do not have to create a [!INCLUDE[ssSB](../../Topics/TopicNameContainA/tokens/ssSB_md.md)]-initiating service because [!INCLUDE[ssSB](../../Topics/TopicNameContainA/tokens/ssSB_md.md)] includes the following specific message type and contract for event notifications:  
...
3.  Create a route on the service to define the address to which [!INCLUDE[ssSB](../../Topics/TopicNameContainA/tokens/ssSB_md.md)] sends messages for the service. For event notifications that target a service in the same database, specify `ADDRESS = 'LOCAL'`.  
  
    > [!NOTE]  
    >  [!INCLUDE[ssSB](../../Topics/TopicNameContainA/tokens/ssSB_md.md)] routing determines the service that receives the notification messages. If the event notification targets a service on a remote server, both the source server and the target server must have routes defined on them to make sure that two-way communication occurs.  
...
## Creating the Event Notification  
 Event notifications are created by using the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] CREATE EVENT NOTIFICATION statement, and are dropped by using the DROP EVENT NOTIFICATION STATEMENT. To modify an event notification, you must drop and re-create the event notification.  
