...
---
# Event Notifications
  Event notifications send information about events to a [!INCLUDE[ssSB](../../Token/Other/ssSB_md.md)] service. Event notifications execute in response to a variety of [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] data definition language \(DDL\) statements and SQL Trace events by sending information about these events to a [!INCLUDE[ssSB](../../Token/Other/ssSB_md.md)] service.  
...
## Event Notifications Benefits  
 ...
 Unlike SQL Trace, event notifications can be used to perform an action inside an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] in response to a SQL Trace event.  
  
 Event data can be used by applications that are running together with [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] to track progress and make decisions. For example, the following event notification sends a notice to a certain service every time an `ALTER TABLE` statement is issued in the [!INCLUDE[ssSampleDBobject](../../Token/Other/ssSampleDBobject_md.md)] sample database.  
...
## Event Notifications Concepts  
 When an event notification is created, one or more [!INCLUDE[ssSB](../../Token/Other/ssSB_md.md)] conversations between an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] and the target service you specify are opened. The conversations typically remain open as long as the event notification exists as an object on the server instance. In some error cases the conversations can close before the event notification is dropped. These conversations are never shared between event notifications. Every event notification has its own exclusive conversations. Ending a conversation explicitly prevents the target service from receiving more messages, and the conversation will not reopen the next time the event notification fires.  
  
 Event information is delivered to the [!INCLUDE[ssSB](../../Token/Other/ssSB_md.md)] service as a variable of type **xml** that provides information about when an event occurs, about the database object affected, the [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] batch statement involved, and other information. For more information about the XML schema produced by event notifications, see [EVENTDATA &#40;Transact-SQL&#41;](../Topic/EVENTDATA%20\(Transact-SQL\).md).  
...
|DML trigger names are schema\-scoped. DDL trigger names are database\-scoped or server\-scoped.|Event notification names are scoped by the server or database. Event notifications on a QUEUE\_ACTIVATION event are scoped to a specific queue.|  
...
|Metadata about triggers is found in the **sys.triggers** and **sys.server\_triggers** catalog views.|Metadata about event notifications is found in the **sys.event\_notifications** and **sys.server\_event\_notifications** catalog views.|  
...
|Trace events are implemented by using system stored procedures.|Event notifications are implemented by using a combination of [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] and [!INCLUDE[ssSB](../../Token/Other/ssSB_md.md)][!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements.|  
|Trace event data can be accessed programmatically by querying the corresponding trace table, parsing the trace file, or using the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Management Objects \(SMO\) TraceReader Class.|Event data is accessed programmatically by issuing XQuery against the XML\-formatted event data, or by using the SMO Event classes.|  
...
|Describes how to configure [!INCLUDE[ssSB](../../Token/Other/ssSB_md.md)] dialog security for event notifications that send messages to a service broker on a remote server.|[Configure Dialog Security for Event Notifications](../../Topics/TopicNameNotContainA/Configure-Dialog-Security-for-Event-Notifications.md)|  
