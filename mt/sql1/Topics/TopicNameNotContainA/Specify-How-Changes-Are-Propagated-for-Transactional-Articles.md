...
-   Specify that the change should be propagated using an INSERT, UPDATE, or DELETE statement (the default for non-[!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Subscribers).  
...
-   You must support the logic in the stored procedure; [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] does not provide support for custom logic.  
...
## Examples  
 The following procedures are the default procedures created for the `Vendor Table` in the [!INCLUDE[ssSampleDBCoShort](../../Token/Other/ssSampleDBCoShort_md.md)] sample database.  
