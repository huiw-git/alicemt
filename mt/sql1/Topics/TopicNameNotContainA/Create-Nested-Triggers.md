...
> [!NOTE]  
>  Any reference to managed code from a [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] trigger counts as one level against the 32\-level nesting limit. Methods invoked from within managed code do not count against this limit.  
...
## Recursive Triggers  
 An AFTER trigger does not call itself recursively unless the RECURSIVE\_TRIGGERS database option is set.  
...
-   Indirect recursion  
 ...
 Only direct recursion of AFTER triggers is prevented when the RECURSIVE\_TRIGGERS database option is set to OFF. To disable indirect recursion of AFTER triggers, also set the **nested triggers** server option to **0**.  
 ...
 **To set the RECURSIVE\_TRIGGERS database option**  
