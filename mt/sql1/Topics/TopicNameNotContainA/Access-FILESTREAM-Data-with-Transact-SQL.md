...
This topic describes how to use the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] INSERT, UPDATE, and DELETE statements to manage FILESTREAM data.  
...
##  <a name="ins"></a> Inserting a Row That Contains FILESTREAM Data  
 To add a row to a table that supports FILESTREAM data, use the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] INSERT statement. When you insert data into a FILESTREAM column, you can insert NULL or a **varbinary(max)** value.  
  
### Inserting NULL  
 The following example shows how to insert `NULL`. When the FILESTREAM value is `NULL`, the [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] does not create a file in the file system.  
...
### Creating a Data File  
 The following example shows how to use `INSERT` to create a file that contains data. The [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] converts the string `Seismic Data` to a `varbinary(max)` value. FILESTREAM creates the Windows file if it does not already exist.The data is then added to the data file.  
...
##  <a name="upd"></a> Updating FILESTREAM Data  
 You can use [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] to update the data in the file system file; although, you might not want to do this when you have to stream large amounts of data to a file.  
...
##  <a name="del"></a> Deleting FILESTREAM Data  
 When you delete a row that contains a FILESTREAM field, you also delete its underlying file system files. The only way to delete a row, and therefore the file, is to use the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] DELETE statement.  
