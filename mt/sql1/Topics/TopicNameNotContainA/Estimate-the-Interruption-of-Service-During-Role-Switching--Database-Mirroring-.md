...
-   In [!INCLUDE[ssStandard](../../Topics/TopicNameNotContainA/tokens/ssStandard_md.md)], the mirror server always uses a single thread to roll forward the database.  
  
-   In [!INCLUDE[ssEnterprise](../../Topics/TopicNameContainA/tokens/ssEnterprise_md.md)], mirror servers on computers with fewer than five CPUs also use only a single thread. With five or more CPUs, a mirror server distributes its roll forward operations among multiple threads during a failover (this is known as *parallel redo*). Parallel redo is optimized to use one thread for every four CPUs.  
...
## Estimating Interruption of Service During Automatic Failover  
 ...
 ![Error detection and failover time](../../Topics/TopicNameNotContainA/images/DBM_failovauto_time.gif "DBM_failovauto_time")  
