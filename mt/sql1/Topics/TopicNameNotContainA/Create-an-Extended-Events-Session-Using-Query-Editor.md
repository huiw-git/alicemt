...
1.  The following procedure shows how to create an Extended Events session by using Query Editor in [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/tokens/ssManStudioFull_md.md)].  
...
4.  (Optional) After you add an event, you can add actions to take. You can also add predicates. Predicates are used to establish criteria for when the event information should be consumed by the target. Actions are added by using an ACTION clause, and predicates are added by using a WHERE clause. For example, to add an action and predicate where the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] text is captured for the sqlserver.file_read_completed event, where the file ID equals 1, you would include the following statement:  
...
-   Event data for completed file reads, including the associated [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] text for file reads where the file ID is equal to 1.  
