...
##  <a name="Start"></a> Start the Database Engine Tuning Advisor  
 You can start the Database Engine Tuning Advisor graphical user interface \(GUI\) in several different ways to support database tuning in a variety of scenarios. The different ways to start Database Engine Tuning Advisor include: from the **Start** menu, from the **Tools** menu in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], from the Query Editor in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], and from the **Tools** menu in [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)]. When you first start Database Engine Tuning Advisor, the application displays a **Connect to Server** dialog box where you can specify the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] instance to which you want to connect.  
  
> [!WARNING]  
>  Do not start Database Engine Tuning Advisor when [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] is running in single\-user mode. If you attempt to start it while the server is in single\-user mode, an error will be returned and Database Engine Tuning Advisor will not start. For more information about single\-user mode, see [Start SQL Server in Single-User Mode](../../Topics/TopicNameNotContainA/Start-SQL-Server-in-Single-User-Mode.md).  
...
1.  On the [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] **Tools** menu, click **Database Engine Tuning Advisor**.  
  
#### To start the Database Engine Tuning Advisor from the SQL Server Management Studio Query Editor  
  
1.  Open a [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script file in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)]. For more information, see [Query and Text Editors &#40;SQL Server Management Studio&#41;](../../Topics/TopicNameNotContainA/Query-and-Text-Editors--SQL-Server-Management-Studio-.md).  
  
2.  Select a query in the [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script, or select the entire script, right\-click the selection, and choose **Analyze Query in Database Engine Tuning Advisor**. The Database Engine Tuning Advisor GUI opens and imports the script as an XML file workload. You can specify a session name and tuning options to tune the selected [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] queries as your workload.  
...
##  <a name="Create"></a> Create a Workload  
 A workload is a set of [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements that execute against a database or databases that you want to tune. Database Engine Tuning Advisor analyzes these workloads to recommend indexes or partitioning strategies that will improve your server's query performance.  
...
-   Use the Query Editor in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] or your favorite text editor to manually create [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script workloads.  
  
-   Use [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] to create trace file or trace table workloads  
...
1.  Launch the Query Editor in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)]. For more information, see [Query and Text Editors &#40;SQL Server Management Studio&#41;](../../Topics/TopicNameNotContainA/Query-and-Text-Editors--SQL-Server-Management-Studio-.md).  
  
2.  Type your [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script into the Query Editor. This script should contain a set of [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements that execute against the database or databases that you want to tune.  
  
3.  Save the file with a **.sql** extension. The Database Engine Tuning Advisor GUI and the command\-line **dta** utility can use this [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script as a workload.  
  
###  <a name="Profiler"></a> To create trace file and trace table workloads  
  
1.  Launch [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] by using one of the following methods:  
  
    -   On the **Start** menu, point to **All Programs**, **Microsoft SQL Server**, **Performance Tools**, and then click **SQL Server Profiler**.  
  
    -   In [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)], click the **Tools** menu, and then click **SQL Server Profiler**.  
  
2.  Create a trace file or table as described in the following procedures that uses the [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] **Tuning** template:  
...
1.  Launch Database Engine Tuning Advisor, and log into an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. For more information, see [Start the Database Engine Tuning Advisor](#Start) earlier in this topic.  
...
3.  Launch Database Engine Tuning Advisor, and log into an instance of [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. For more information, see [Start the Database Engine Tuning Advisor](#Start) earlier in this topic.  
...
##  <a name="XMLInput"></a> Create an XML Input File  
 If you are an experienced XML developer, you can create XML\-formatted files that [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor can use to tune workloads. To create these XML files, use your favorite XML tools to edit a sample file or to generate an instance from the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor XML schema.  
  
 The [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor XML schema is available in your [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] installation in the following location:  
  
 C:\\Program Files\\Microsoft SQL Server\\100\\Tools\\Binn\\schemas\\sqlserver\\2004\\07\\dta\\dtaschema.xsd  
  
 The [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor XML schema is also available online at this [Microsoft Web site](http://go.microsoft.com/fwlink/?linkid=43100&clcid=0x409).  
  
 This URL takes you to a page where many [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] XML schemas are available. Scroll down the page until you reach the row for [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor.  
  
#### To create an XML input file to tune workloads  
  
1.  Create a workload. You can use a trace file or table by using the tuning template in [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)], or create a [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script that reproduces a representative workload for [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. For more information, see [Create a Workload](#Create) earlier in this topic.  
  
2.  Create an XML input file by one of the following methods:  
  
    -   Copy and paste one of the [XML Input File Samples &#40;DTA&#41;](../../Topics/TopicNameNotContainA/XML-Input-File-Samples--DTA-.md) into your favorite XML editor. Change the values to specify the appropriate arguments for your [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] installation, and save the XML file.  
  
    -   Using your favorite XML tool, generate an instance from the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor XML schema.  
 ...
 **Xml**  
 This does not appear unless you import a workload query from [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)].  
  
 To import a workload query from [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)]:  
 ...
 **Preview the XML workload**  
 View an XML\-formatted workload that has been imported from [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)].  
...
> [!NOTE]  
>  [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor consumes system resources during analysis. Use **Limit tuning time** to stop tuning before periods of anticipated heavy workload on the server being tuned.  
 ...
 **Generate online recommendations where possible**  
 When creating [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements to implement the recommendations, choose methods that can be implemented with the server online, even if a faster offline method is available.  
  
 **Generate only online recommendations**  
 Only make recommendations that allow the server to stay online.  
  
 **Stop at**  
 Provide the date and time when [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor should stop.  
 ...
 **Do not keep any existing PDS**  
 Recommend dropping unnecessary existing indexes, views, and partitioning. If an existing physical design structure \(PDS\) is useful to the workload, [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] Tuning Advisor does not recommend dropping it.  
