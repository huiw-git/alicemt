...
|CREATE INDEX WITH DROP\_EXISTING|Disabled clustered index or disabled indexed view<br /><br /> Index on a local temp table<br /><br /> XML index||  
...
|ALTER TABLE DROP CONSTRAINT \(PRIMARY KEY or UNIQUE\)|Clustered index||  
 ...
 The online option setting \(ON or OFF\) specified when you create or drop a clustered index is applied to any nonclustered indexes that must be rebuilt. For example, if the clustered index is built online by using CREATE INDEX WITH DROP\_EXISTING, ONLINE\=ON, all associated nonclustered indexes are re\-created online also.  
 ...
 Although not common, the online index operation can cause a deadlock when it interacts with database updates because of user or application activities. In these rare cases, the [!INCLUDE[ssDEnoversion](../../Token/Other/ssDEnoversion_md.md)] will select the user or application activity as a deadlock victim.  
