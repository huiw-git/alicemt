...
2.  Record all service packs and hotfixes applied to the instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] and the current collation. You must reapply these updates after rebuilding the system databases.  
...
5.  If the instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] is configured as a replication Distributor, locate the current backup of the distribution database.  
...
2.  From a command prompt window, enter the following command. Square brackets are used to indicate optional parameters. Do not enter the brackets. When using a Windows operating system that has User Account Control (UAC) enabled, running Setup requires elevated privileges. The command prompt must be run as Administrator.  
    ...
    |/INSTANCENAME=*InstanceName*|Is the name of the instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)]. For the default instance, enter MSSQLSERVER.|  
    ...
    |[ /SAPWD=*StrongPassword* ]|Specifies the password for the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] **sa** account. This parameter is required if the instance uses Mixed Authentication ([!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] and Windows Authentication) mode.<br /><br /> **\*\* Security Note \*\***The **sa** account is a well-known [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] account and it is often targeted by malicious users. It is very important that you use a strong password for the **sa** login.<br /><br /> Do not specify this parameter for Windows Authentication mode.|  
...
1.  Launch the [!INCLUDE[ssCurrent](../../Topics/TopicNameContainA/includes/ssCurrent_md.md)] Setup program (setup.exe) from the distribution media.  
...
1.  Stop all services connecting to the [!INCLUDE[ssDE](../../Topics/TopicNameContainA/includes/ssDE_md.md)], including [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] Agent, [!INCLUDE[ssRS](../../Topics/TopicNameNotContainA/includes/ssRS_md.md)], [!INCLUDE[ssIS](../../Topics/TopicNameContainA/includes/ssIS_md.md)], and all applications using [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] as data store.  
  
2.  Start [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] from the command line using the command: `NET START MSSQLSERVER /T3608`  
...
3.  In another command line window, detach the **msdb** database by executing the following command, replacing *<servername\>* with the instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)]: `SQLCMD -E -S<servername> -dmaster -Q"EXEC sp_detach_db msdb"`  
  
4.  Using the Windows Explorer, rename the **msdb** database files. By default these are in the DATA sub-folder for the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] instance.  
  
5.  Using [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] Configuration Manager, stop and restart the [!INCLUDE[ssDE](../../Topics/TopicNameContainA/includes/ssDE_md.md)] service normally.  
  
6.  In a command line window, connect to [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)] and execute the command: `SQLCMD -E -S<servername> -i"C:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\MSSQL\Install\instmsdb.sql" -o" C:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\MSSQL\Install\instmsdb.out"`  
  
     Replace *<servername\>* with the instance of the [!INCLUDE[ssDE](../../Topics/TopicNameContainA/includes/ssDE_md.md)]. Use the file system path of the instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/includes/ssNoVersion_md.md)].  
