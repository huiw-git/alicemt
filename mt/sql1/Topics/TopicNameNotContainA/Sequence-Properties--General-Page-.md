...
# Sequence Properties (General Page)
 ...
 Unlike identity columns values which are generated at the time of insert, an application can obtain the next sequence number without inserting the row by calling the [NEXT VALUE FOR function](../Topic/NEXT%20VALUE%20FOR%20\(Transact-SQL\).md). Use [sp\_sequence\_get\_range](../Topic/sp_sequence_get_range%20\(Transact-SQL\).md) to get multiple sequence numbers at once.  
...
-   Default cache size \- The [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] will select a size, however users should not rely upon the selection being consistent. [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] might change the method of calculating the cache size without notice.  
  
-   No cache \- [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] will not cache sequence numbers.  
  
-   Cache with size \- [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] will cache sequence values. [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] keeps track of the current value and the number of values left in the cache. Therefore, the amount of memory that is required to store the cache is always two instances of the data type of the sequence object  
