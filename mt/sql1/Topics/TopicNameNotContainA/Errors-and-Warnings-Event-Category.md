---
title: "Errors and Warnings Event Category"
ms.custom: na
ms.date: 06/29/2016
ms.prod: sql-server-2016
ms.reviewer: na
ms.suite: na
ms.technology: 
  - analysis-services
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: cf7fd5a7-b189-47c3-8e5c-3195897c7eab
caps.latest.revision: 21
caps.handback.revision: 0
manager: mblythe
---
# Errors and Warnings Event Category
The Errors and Warnings event category has the event classes described in the following table.  
  
|Event Class|Event Id|Description|  
|-----------------|--------------|-----------------|  
|Error|17|Records all new error events since the trace started.|  
  
 For information about the columns associated with each of the Errors and Warnings event classes, see [Errors and Warnings Events Data Columns](../../Topics/TopicNameNotContainA/Errors-and-Warnings-Events-Data-Columns.md).  
  
## See Also  
 [Analysis Services Trace Events](../../Topics/TopicNameNotContainA/Analysis-Services-Trace-Events.md)