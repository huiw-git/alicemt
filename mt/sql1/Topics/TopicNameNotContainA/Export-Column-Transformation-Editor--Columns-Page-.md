 ...
 **Write BOM**  
 Specify whether to write a byte\-order mark \(BOM\) to the file. A BOM is only written if the data has the **DT\_NTEXT** or DT\_WSTR data type and is not appended to an existing data file.  
