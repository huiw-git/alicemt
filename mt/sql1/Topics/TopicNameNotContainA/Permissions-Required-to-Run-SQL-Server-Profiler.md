...
# Permissions Required to Run SQL Server Profiler
  By default, running [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] requires the same user permissions as the Transact\-SQL stored procedures that are used to create traces. To run [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)], users must be granted the ALTER TRACE permission. For more information, see [GRANT Server Permissions &#40;Transact-SQL&#41;](../Topic/GRANT%20Server%20Permissions%20\(Transact-SQL\).md).  
...
## Permissions Used to Replay Traces  
 ...
 However, during replay, [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] uses the EXECUTE AS command if an Audit Login event is encountered in the trace that is being replayed. [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] uses the EXECUTE AS command to impersonate the user who is associated with the login event.  
  
 If [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] encounters a login event in a trace that is being replayed, the following permission checks are performed:  
...
3.  [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] uses the EXECUTE AS command to impersonate User2.  
  
4.  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] attempts to authenticate User2, and depending on the results, one of the following occurs:  
  
    1.  If User2 cannot be authenticated, [!INCLUDE[ssSqlProfiler](../../Token/Other/ssSqlProfiler_md.md)] returns an error, and continues replaying the trace as User1.  
...
6.  Existence of a Guest user is checked on the target database, and depending on the results, one of the following occurs:  
 ...
 ![SQL Server Profiler replay trace permissions](../../Images/Image/ImageNotContaina/replayTraceDecisionTree.gif "replayTraceDecisionTree")  
