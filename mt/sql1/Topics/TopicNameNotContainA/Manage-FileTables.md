...
-   [sys.filetables (Transact-SQL)](assetId:///a740be59-cd52-4707-9ad2-5203669a63ac)  
  
-   [sys.tables (Transact-SQL)](assetId:///8c42eba1-c19f-4045-ac82-b97a5e994090) (Check the value of the **is_filetable** column.)  
...
```  
 ...
 To get a list of the system-defined objects that were created when the associated FileTables were created, query the catalog view [sys.filetable_system_defined_objects](assetId:///62022e6b-46f6-495f-b14b-53f41e040361).  
...
###  <a name="HowToDisable"></a> How To: Disable and Re-enable Non-Transactional Access at the Database Level  
 For more information, see [ALTER DATABASE SET Options (Transact-SQL)](assetId:///f76fbd84-df59-4404-806b-8ecb4497c9cc).  
...
##  <a name="BasicsEnabling"></a> Disabling and Re-enabling the FileTable Namespace at the Table Level  
 Disabling the FileTable namespace disables all the system-defined constraints and triggers that were created with the FileTable. This is useful in cases where a FileTable has to be reorganized on a large scale by using [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] operations without incurring the expense of enforcing FileTable semantics. However these operations can leave the FileTable in an inconsistent state, and can prevent the re-enabling of the FileTable namespace.  
...
###  <a name="HowToListOpen"></a> How To: Get a List of Open File Handles Associated with a FileTable  
 Query the catalog view [sys.dm_filestream_non_transacted_handles](assetId:///507ec125-67dc-450a-9081-94cde5444a92).  
...
###  <a name="HowToKill"></a> How To: Kill Open File Handles Associated with a FileTable  
 Call the stored procedure [sp_kill_filestream_non_transacted_handles (Transact-SQL)](assetId:///7188353e-ab29-49a0-8f25-7fb8ab122589) with the appropriate arguments to kill all open file handles in the database or in the FileTable, or to kill a specific handle.  
 ...
 **To identify open files and the associated locks**  
 Join the **request_owner_id** field in the dynamic management view [sys.dm_tran_locks (Transact-SQL)](assetId:///f0d3b95a-8a00-471b-9da4-14cb8f5b045f) with the **fcb_id** field in [sys.dm_filestream_non_transacted_handles (Transact-SQL)](assetId:///507ec125-67dc-450a-9081-94cde5444a92). In some cases, the lock does not correspond to a single open file handle.  
...
##  <a name="BasicsSecurity"></a> FileTable Security  
 The files and directories stored in FileTables are secured by SQL Server security only. Table and column-based security is enforced for file system access as well as [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] access. Windows file system security APIs and ACL settings are not supported.  
 ...
 **FileTable Security and Transact-SQL Access**  
 [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] access to data in FileTables is secured in the same way as any other table. Appropriate table and column-level security checks are done for every operation that accesses or changes the data.  
  
 **FileTable Security and File System Access**  
 File system APIs require appropriate [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] permissions on the entire row in the FileTable (that is, table-level permission) to open a handle to a file or directory stored in the FileTable. If the user does not have the appropriate [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] permission on any column in the FileTable, then file system access is denied.  
  
##  <a name="OtherBackup"></a> Backup and FileTables  
 When you use [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] to back up a FileTable, the FILESTREAM data is backed up with the structured data in the database. If you do not want to back up FILESTREAM data with relational data, you can use a partial backup to exclude FILESTREAM filegroups.  
...
##  <a name="Monitor"></a> SQL Server Profiler and FileTables  
 [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Profiler can capture the Windows File Open and File Close operations in trace output for files that are stored in a FileTable.  
