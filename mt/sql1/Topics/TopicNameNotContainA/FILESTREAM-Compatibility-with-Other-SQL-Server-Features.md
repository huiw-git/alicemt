...
Because FILESTREAM data is in the file system, this topic provides some considerations, guidelines, and limitations for using FILESTREAM with the following features in [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)]:  
...
##  <a name="DatabaseSnapshot"></a> Database Snapshots  
 [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] does not support [database snapshots](../../Topics/TopicNameNotContainA/Database-Snapshots--SQL-Server-.md) for FILESTREAM filegroups. If a FILESTREAM filegroup is included in a CREATE DATABASE ON clause, the statement will fail and an error will be raised.  
...
##  <a name="Replication"></a> Replication  
 A **varbinary(max)** column that has the FILESTREAM attribute enabled at the Publisher can be replicated to a Subscriber with or without the FILESTREAM attribute. To specify the way in which the column is replicated, use the **Article Properties - <Article\>** dialog box or the @schema_option parameter of [sp_addarticle](assetId:///0483a157-e403-4fdb-b943-23c1b487bef0) or [sp_addmergearticle](assetId:///0df654ea-24e2-4c61-a75a-ecaa7a140a6c). Data that is replicated to a **varbinary(max)** column that does not have the FILESTREAM attribute must not exceed the 2-GB limit for that data type; otherwise, a run-time error is generated. We recommend that you replicate the FILESTREAM attribute, unless you are replicating data to [!INCLUDE[ssVersion2005](../../Topics/TopicNameContainA/tokens/ssVersion2005_md.md)]. Replicating tables that have FILESTREAM columns to [!INCLUDE[ssVersion2000](../../Topics/TopicNameContainA/tokens/ssVersion2000_md.md)] Subscribers is not supported, regardless of the schema option that is specified.  
  
> [!NOTE]  
>  Replicating large data values from [!INCLUDE[ssCurrent](../../Topics/TopicNameContainA/tokens/ssCurrent_md.md)] to [!INCLUDE[ssVersion2005](../../Topics/TopicNameContainA/tokens/ssVersion2005_md.md)] Subscribers is limited to a maximum of 256 MB data values. For more information, see [Maximum Capacity Specifications](http://go.microsoft.com/fwlink/?LinkId=103810).  
...
-   If any tables include columns that have the FILESTREAM attribute, you cannot use values of *database snapshot* or *database snapshot character* for the @sync_method property of [sp_addpublication](assetId:///c7167ed1-2b7e-4824-b82b-65f4667c4407).  
...
-   Merge replication includes an optimization for replicating large object types. This optimization is controlled by the @stream_blob_columns parameter of [sp_addmergearticle](assetId:///0df654ea-24e2-4c61-a75a-ecaa7a140a6c). If you set the schema option to replicate the FILESTREAM attribute, the @stream_blob_columns parameter value is set to **true**. This optimization can be overridden by using [sp_changemergearticle](assetId:///0dc3da5c-4af6-45be-b5f0-074da182def2). This stored procedure enables you to set @stream_blob_columns to **false**. If you add a FILESTREAM column to a table that is already published for merge replication, we recommend that you set the option to **true** by using sp_changemergearticle.  
...
##  <a name="LogShipping"></a> Log Shipping  
 [Log shipping](../../Topics/TopicNameNotContainA/About-Log-Shipping--SQL-Server-.md) supports FILESTREAM. Both the primary and secondary servers must be running [!INCLUDE[ssKatmai](../../Topics/TopicNameContainA/tokens/ssKatmai_md.md)], or a later version, and have FILESTREAM enabled.  
...
##  <a name="SQLServerExpress"></a> SQL Server Express  
 [!INCLUDE[ssExpress](../../Topics/TopicNameContainA/tokens/ssExpress_md.md)] supports FILESTREAM. The 10-GB database size limit does not include the FILESTREAM data container.  
