...
-   Be aware of the DML and DDL triggers that exist in the database and on the server instance by querying the [sys.triggers](../Topic/sys.triggers%20\(Transact-SQL\).md) and [sys.server\_triggers](../Topic/sys.server_triggers%20\(Transact-SQL\).md) catalog views. The following query returns all DML and database\-level DDL triggers in the current database, and all server\-level DDL triggers on the server instance:  
