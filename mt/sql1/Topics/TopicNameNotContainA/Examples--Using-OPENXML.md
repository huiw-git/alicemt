...
### A. Executing a simple SELECT statement with OPENXML  
 ...
 First, the **sp\_xml\_preparedocument** stored procedure is called to obtain a document handle. This document handle is passed to OPENXML.  
...
```  
 ...
 Note that the document handle returned by **sp\_xml\_preparedocument** is valid for the duration of the batch and not the session.  
...
### G. Specifying rowpattern ending with an attribute  
 ...
 First, the **sp\_xml\_preparedocument** is called to obtain a document handle. This document handle is passed to OPENXML.  
...
```  
 ...
 Specifically, you are passing an **xml** type variable \(@x\) to the **sp\_xml\_preparedocument\(\)** function.  
