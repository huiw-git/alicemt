...
# Understanding DAX in Tabular Models (SSAS Tabular)
  Data Analysis Expressions \(DAX\) is the formula language used to create custom calculations in [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for Microsoft Excel workbooks and [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] tabular model projects. DAX formulas include functions, operators, and values to perform advanced calculations on data in tables and columns.  
  
 While DAX applies to both [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks and tabular model projects, this topic applies more to tabular model projects authored in [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)]. Before reading this topic, you should have a good understanding of tabular models and the tabular model project authoring environment in [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)].  
...
##  <a name="bkmk_DAXintm"></a> DAX in Tabular Models  
 In both [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] and tabular models, functionally, there is no difference in how DAX formulas calculate values from their respective data sets. However, where DAX formulas are created in the workbook and model authoring tools is different, as well as where context in certain measures is evaluated.  
  
 In [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)], calculation formulas are typically created by the workbook user for self\-service business intelligence analysis. Calculated columns are created for a table in the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] window, and measures are created in PivotTables or the calculation area. Unlike tabular model projects, [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks do not provide role\-based security, which can use DAX formulas to secure data.  
  
 In tabular model projects, calculation formulas are created in the model designer in [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)] by model authors. While values for calculated columns, calculated by using DAX formulas, appear immediately in the table in the model designer, with exception to the measure preview feature in the measure grid, measures are not calculated until a user specifies a filter in a reporting client such as [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)] or in PivotTables in Microsoft Excel.  
  
 If you import a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook into a new tabular model project by using the Import from [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] project template, DAX formulas for calculated columns will be automatically created in the new tabular model. DAX formulas for implicit and explicit measures in the workbook will be automatically created in the new tabular model as explicit measures. Because role and secure row filter functionality do not already exist in [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks, you will need to create at least one role in the new tabular model to provide model data access to role members. DAX formulas in row filters are necessary only if you want to secure table data at the row\-level.  
  
##  <a name="bkmk_DAX"></a> DAX Formulas in Calculated Columns, Measures, and Row Filters  
 For tabular models authored in [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)], DAX formulas are used in calculated columns, measures, and row filters.  
...
### Measures  
 Measures are dynamic formulas where the results change depending on context. Measures are used in reporting formats that support combining and filtering model data by using multiple attributes such as a [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)] report or Excel PivotTable or PivotChart. In tabular model projects, measures are defined by the model author by using the measure grid \(and formula bar\) in the model designer in [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)].  
...
### Row Filters  
 Row filters define which rows in a table are visible to members of a particular role. Row filters can be created for each table in a model by using DAX formulas. Row filters are created for a particular role by using Role Manager in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)]. Row filters can also be defined for a deployed model by using Role Properties in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)].  
 ...
 *Filter context* is the set of values allowed in each column, or in the values retrieved from a related table. Filters can be applied to the column in the designer, or in the presentation layer \(reports and PivotTables\). Filters can also be defined explicitly by filter expressions within the formula.  
 ...
 In tabular models, there are many ways to create filter context. Within the context of clients that can consume the model, such as [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)] reports, users can create filters on the fly by adding slicers or report filters on the row and column headings. You can also specify filter expressions directly within the formula, to specify related values, to filter tables that are used as inputs, or to dynamically get context for the values that are used in calculations. You can also completely clear or selectively clear the filters on particular columns. This is very useful when creating formulas that calculate grand totals.  
...
##  <a name="bkmk_addional_resources"></a> Additional Resources  
 ...
 The [Analysis Services and Power Pivot Team Blog](http://go.microsoft.com/fwlink/?LinkID=220949&clcid=0x409) provides information, tips, news and announcements about [!INCLUDE[ssASCurrent](../../Token/Other/ssASCurrent_md.md)] and [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)].  
