...
# Row Compression Implementation
  This topic summarizes how [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] implements row compression. This summary provides basic information to help you plan the storage space that you need for your data.  
...
## How Row Compression Affects Storage  
 The following table describes how row compression affects the existing types in [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. The table does not include the savings that can be achieved by using page compression.  
...
|**char**|Yes|Trailing padding characters are removed. Note that the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] inserts the same padding character regardless of the collation that is used.|  
|**varchar**|No|No effect.|  
|**text**|No|No effect.|  
|**nchar**|Yes|Trailing padding characters are removed. Note that the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] inserts the same padding character regardless of the collation that is used.|  
