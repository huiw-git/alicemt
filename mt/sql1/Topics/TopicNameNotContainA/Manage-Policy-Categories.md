...
# Manage Policy Categories
  This topic describes how to apply any or all available policies in a category to the whole instance of [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] by using [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] or [!INCLUDE[tsql](../../Token/Other/tsql_md.md)].  
...
-   When using [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)], if the **Mandate Database Subscriptions** check box is not selected, the policy category must be individually applied to each relevant portion of the server, such as one or more databases or tables.  
  
-   If you specify a policy category that does not exist, a new policy category is created and the subscription is mandated for all databases when you execute the stored procedure. If you then clear the mandated subscription for the new category, the subscription will only apply for the database that you specified as the *target\_object*. For more information about how to change a mandated subscription setting, see [sp_syspolicy_update_policy_category &#40;Transact-SQL&#41;](../Topic/sp_syspolicy_update_policy_category%20\(Transact-SQL\).md).  
...
3.  Right\-click **Policy Management** and select **Manage Categories**.  
     ...
     **Mandate Database Subscriptions**  
     Forces all databases on the instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] to enforce policies in the policy category.  
