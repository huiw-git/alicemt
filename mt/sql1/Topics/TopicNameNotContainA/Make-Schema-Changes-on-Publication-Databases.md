...
Replication supports a wide range of schema changes to published objects. When you make any of the following schema changes on the appropriate published object at a [!INCLUDE[msCoName](../../Topics/TopicNameContainA/tokens/msCoName_md.md)] [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Publisher, that change is propagated by default to all [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Subscribers:  
  
-   ALTER TABLE  
  
-   ALTER TABLE SET LOCK ESCALATION should not be used if schema change replication is enabled and a topology includes [!INCLUDE[ssVersion2005](../../Topics/TopicNameContainA/tokens/ssVersion2005_md.md)] or [!INCLUDE[ssEWnoversion](../../Topics/TopicNameNotContainA/tokens/ssEWnoversion_md.md)] Subscribers.ALTER VIEW  
...
> [!IMPORTANT]  
>  Schema changes to tables must be made by using [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] or [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Management Objects (SMO). When schema changes are made in [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/tokens/ssManStudioFull_md.md)], [!INCLUDE[ssManStudio](../../Topics/TopicNameContainA/tokens/ssManStudio_md.md)] attempts to drop and re-create the table. You cannot drop published objects, therefore the schema change fails.  
...
-   Schema changes are subject to any restrictions imposed by [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)]. For example, ALTER TABLE does not allow you to ALTER primary key columns.  
  
-   Data type mapping is performed only for the initial snapshot. Schema changes are not mapped to previous versions of data types. For example, if the statement `ALTER TABLE ADD datetime2 column` is used in [!INCLUDE[ssSQL11](../../Topics/TopicNameContainA/tokens/ssSQL11_md.md)], the data type is not translated to **nvarchar** for [!INCLUDE[ssVersion2005](../../Topics/TopicNameContainA/tokens/ssVersion2005_md.md)] Subscribers. In some cases, schema changes are blocked on the Publisher.  
...
-   To include an existing column in an existing publication, use [sp_articlecolumn (Transact-SQL)](assetId:///8abaa8c1-d99e-4788-970f-c4752246c577), [sp_mergearticlecolumn (Transact-SQL)](assetId:///b4f2b888-e094-4759-a472-d893638995eb), or the **Publication Properties - <Publication\>** dialog box.  
...
-   To drop a column from an existing publication but retain the column in the table at the Publisher, use [sp_articlecolumn (Transact-SQL)](assetId:///8abaa8c1-d99e-4788-970f-c4752246c577), [sp_mergearticlecolumn (Transact-SQL)](assetId:///b4f2b888-e094-4759-a472-d893638995eb), or the **Publication Properties - <Publication\>** dialog box.  
...
-   Schema changes are propagated to Subscribers running previous versions of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)], but the DDL statement should only include syntax supported by the version at the Subscriber.  
  
     If the Subscriber republishes data, the only supported schema changes are adding and dropping a column. These changes should be made on the Publisher using [sp_repladdcolumn](assetId:///d6220f9f-c738-4f9c-bcf8-419994e86c81) and [sp_repldropcolumn](assetId:///fdc1ec5f-f108-42b4-a2d8-f06a71913ab8) rather than ALTER TABLE DDL syntax.  
  
-   Schema changes are not replicated to non-SQL Server Subscribers.  
  
-   Schema changes are not propagated from non-[!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Publishers.  
...
-   If the publication is in a peer-to-peer topology, the system must be quiesced before making schema changes. For more information, see [Quiesce a Replication Topology (Replication Transact-SQL Programming)](../../Topics/TopicNameContainA/Quiesce-a-Replication-Topology--Replication-Transact-SQL-Programming-.md).  
...
-   How merge replication handles schema changes is determined by the publication compatibility level, and whether the snapshot is set to native mode (default) or character mode:  
    ...
    -   To replicate schema changes, the compatibility level of the publication must be at least 90RTM. If Subscribers are running previous versions of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] or the compatibility level is less than 90RTM you can use [sp_repladdcolumn](assetId:///d6220f9f-c738-4f9c-bcf8-419994e86c81) and [sp_repldropcolumn](assetId:///fdc1ec5f-f108-42b4-a2d8-f06a71913ab8) to add and drop columns. However, these procedures are deprecated.  
  
    -   If you try to add to an existing article a column with a data type that was introduced in [!INCLUDE[ssKatmai](../../Topics/TopicNameContainA/tokens/ssKatmai_md.md)], [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] has the following behavior:  
...
-   Merge replication provides stored procedures to skip schema changes during troubleshooting. For more information, see [sp_markpendingschemachange (Transact-SQL)](assetId:///01100309-7bef-4154-85bf-f18489577e37) and [sp_enumeratependingschemachanges (Transact-SQL)](assetId:///df169b21-d10a-41df-b3a1-654cfb58bc21).  
  
## See Also  
 [ALTER TABLE](assetId:///f1745145-182d-4301-a334-18f799d361d1)   
 [ALTER VIEW](assetId:///03eba220-13e2-49e3-bd9d-ea9df84dc28c)   
 [ALTER PROCEDURE](assetId:///ed9b2f76-11ec-498d-a95e-75b490a75733)   
 [ALTER FUNCTION](assetId:///89f066ee-05ac-4439-ab04-d8c3d5911179)   
