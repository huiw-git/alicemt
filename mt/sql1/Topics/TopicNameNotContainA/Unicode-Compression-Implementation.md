...
# Unicode Compression Implementation
  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] uses an implementation of the Standard Compression Scheme for Unicode \(SCSU\) algorithm to compress Unicode values that are stored in row or page compressed objects. For these compressed objects, Unicode compression is automatic for **nchar\(n\)** and **nvarchar\(n\)** columns. The [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] stores Unicode data as 2 bytes, regardless of locale. This is known as UCS\-2 encoding. For some locales, the implementation of SCSU compression in SQL Server can save up to 50 percent in storage space.  
...
## Upgrading from Earlier Versions of SQL Server  
 When a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database is upgraded to [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)], Unicode compression–related changes are not made to any database object, compressed or uncompressed. After the database is upgraded, objects are affected as follows:  
