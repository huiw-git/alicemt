---
title: "Configure IIS for Web Synchronization"
ms.custom: na
ms.date: 07/04/2016
...
-   [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/tokens/ssManStudioFull_md.md)]: [How to: Configure a Subscription to Use Web Synchronization \(SQL Server Management Studio\)](http://msdn.microsoft.com/library/ms345214.aspx)  
  
-   Replication [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] programming: [How to: Configure a Subscription to Use Web Synchronization (Replication Transact-SQL Programming)](http://msdn.microsoft.com/library/ms345206.aspx)  
...
-   RMO: [How to: Configure a Subscription to Use Web Synchronization (RMO Programming)](http://msdn.microsoft.com/library/ms345207.aspx)  
 ...
 Web synchronization uses a computer that is running [!INCLUDE[msCoName](../../Topics/TopicNameContainA/tokens/msCoName_md.md)] Internet Information Services (IIS) to synchronize pull subscriptions to merge publications. IIS version 5.0, IIS version 6.0, and [!INCLUDE[iisver](../../Topics/TopicNameNotContainA/tokens/iisver_md.md)] are supported. The Configure Web Synchronization Wizard is not supported on [!INCLUDE[iisver](../../Topics/TopicNameNotContainA/tokens/iisver_md.md)].  
  
> [!IMPORTANT]  
>  Make sure that your application uses only [!INCLUDE[dnprdnlong](../../Topics/TopicNameNotContainA/tokens/dnprdnlong_md.md)] or later versions, and that earlier versions of the [!INCLUDE[dnprdnshort](../../Topics/TopicNameContainA/tokens/dnprdnshort_md.md)] are not installed on the IIS server. Earlier versions of the [!INCLUDE[dnprdnshort](../../Topics/TopicNameContainA/tokens/dnprdnshort_md.md)] can cause errors. These include the following: "The format of a message during Web synchronization was invalid. Ensure that replication components are properly configured at the Web server".  
...
2.  Install [!INCLUDE[msCoName](../../Topics/TopicNameContainA/tokens/msCoName_md.md)] [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] connectivity components on the computer that is running IIS by using the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Installation Wizard. If you plan to use the Configure Web Synchronization Wizard that is mentioned in step 3, you must also install [!INCLUDE[ssManStudioFull](../../Topics/TopicNameContainA/tokens/ssManStudioFull_md.md)] on the computer that is running IIS.  
...
4.  Set the appropriate permissions for the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Replication Listener.  
...
## Configuring the Computer That Is Running IIS by Using the Configure Web Synchronization Wizard  
 Configure the IIS server by using the Configure Web Synchronization Wizard or manually. We recommend using the wizard, but we also provide steps for manual configuration in the next section. The Web Synchronization Wizard that is available with [!INCLUDE[ssSQL11](../../Topics/TopicNameContainA/tokens/ssSQL11_md.md)] can be used only for publications that were created on a Publisher that is running [!INCLUDE[ssSQL11](../../Topics/TopicNameContainA/tokens/ssSQL11_md.md)], or a Publisher that was upgraded to [!INCLUDE[ssSQL11](../../Topics/TopicNameContainA/tokens/ssSQL11_md.md)]. The wizard cannot be used for publications on [!INCLUDE[ssVersion2005](../../Topics/TopicNameContainA/tokens/ssVersion2005_md.md)]. The wizard can be used with subscriptions on [!INCLUDE[ssVersion2005](../../Topics/TopicNameContainA/tokens/ssVersion2005_md.md)] and later versions, and [!INCLUDE[ssEWnoversion](../../Topics/TopicNameNotContainA/tokens/ssEWnoversion_md.md)] 3.0 and later versions.  
...
6.  On the **Virtual Directory Information** page:  
    ...
    3.  On both dialog boxes, click **Yes**. This specifies that you want to create a new folder and that you want to copy the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Internet Server API (ISAPI) DLL. .  
...
11. If the computer that is running IIS is running on a 64-bit version of Windows, replisapi.dll must be copied to the appropriate directory:  
    ...
    2.  After IIS stops and restarts, copy replisapi.dll from [!INCLUDE[ssInstallPathVar](../../Topics/TopicNameContainA/tokens/ssInstallPathVar_md.md)]COM\replisapi to the directory that is specified in step 6b.  
...
## Manually Configuring the Computer That Is Running IIS  
 To configure the computer that is running IIS manually, you must install and configure the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Replication Listener, and then configure authorization for Subscribers that will connect to IIS.  
...
1.  Create a file directory on the computer that is running IIS to contain replisapi.dll. You can create the directory wherever you want, but we recommend that you create the directory under the <*drive*>:\Inetpub directory. For example, create the directory <*drive*>:\Inetpub\SQLReplication\\.  
    ...
    > [!IMPORTANT]  
    >  We strongly recommend that you create this directory on an NTFS file system partition instead of a FAT file system. When you use the NTFS file system, you can use NTFS file system permissions to control precisely the users that can access [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] replication.  
  
2.  Copy replisapi.dll from the directory [!INCLUDE[ssInstallPathVar](../../Topics/TopicNameContainA/tokens/ssInstallPathVar_md.md)]com\ to the file directory that you created in step 1.  
...
## Setting Permissions for the SQL Server Replication Listener  
 When a Subscriber connects to the computer that is running IIS, the Subscriber is authenticated by using the type of authentication that was specified when you configured IIS. After IIS authenticates the Subscriber, IIS checks whether the Subscriber is authorized to invoke [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] replication. You control the users that can invoke [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] replication by setting permissions for replisapi.dll. Properly configuring permissions is necessary to prevent unauthorized access to [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] replication.  
  
 To configure the minimum permissions for the account under which the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Replication Listener runs, complete the following procedure. The steps in the procedure apply to [!INCLUDE[msCoName](../../Topics/TopicNameContainA/tokens/msCoName_md.md)] [!INCLUDE[winxpsvr](../../Topics/TopicNameContainA/tokens/winxpsvr_md.md)] running IIS 6.0.  
...
1.  Make sure that local area network (LAN) settings at the Subscriber are correct:  
    ...
    1.  In [!INCLUDE[msCoName](../../Topics/TopicNameContainA/tokens/msCoName_md.md)] Internet Explorer, on the **Tools** menu, click **Internet Options**.  
