---
title: Best Practice with the Query Store
H1: na
ms.custom: 
  - SQL2016_New_Updated
...
##  <a name="SSMS"></a> Use the Latest SQL Server Management Studio  
 [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] has set of user interfaces designed for configuring Query Store as well as for consuming collected data about your workload.  
Download the latest version of [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] from : [https:\/\/msdn.microsoft.com\/library\/mt238290.aspx](https://msdn.microsoft.com/library/mt238290.aspx)  
...
##  <a name="Insight"></a> Use Query Performance Insight in Azure SQL Database  
 If you run Query Store in [!INCLUDE[ssSDS](../../Token/Other/ssSDS_md.md)] you can use **Query Performance Insight** to analyze DTU consumption over time.  
While you can use [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] to get detailed resource consumption for all your queries \(CPU, memory, IO, etc.\), Query Performance Insight gives you a quick and efficient way to determine their impact on overall DTU consumption for your database.  
...
> [!IMPORTANT]  
>  When using Elastic Pool, enable Query Store for only the small subset of databases. Enabling Query Store for all databases in an Elastic Pool can lead to excessive resource usage and could make your system unresponsive.  
...
The default parameters are good for a quick start but you should monitor how Query Store behaves over time and adjust its configuration accordingly:  
 ...
 ![query-store-properties](../../Images/Image/ImageNotContaina/query-store-properties.png "query-store-properties")  
...
## How to start with query performance troubleshooting  
 ...
 ![query-store-troubleshooting](../../Images/Image/ImageNotContaina/query-store-troubleshooting.png "query-store-troubleshooting")  
  
 Enable Query Store by using [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] as described in the previous section, or execute the following [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statement:  
...
Navigate to the Query Store sub\-folder under the database node in Object Explorer of [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] to open troubleshooting views for specific scenarios.   
[!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] Query Store views operate with the set of execution metrics, each expressed as any of the following statistic functions:  
...
|CPU time, Duration, Execution Count, Logical Reads, Logical writes, Memory consumption, and Physical Reads|Average, Maximum, Minimum, Standard Deviation, Total|  
 ...
 ![query-store-views](../../Images/Image/ImageNotContaina/query-store-views.png "query-store-views")  
...
> [!TIP]  
>  For a detailed description how to use [!INCLUDE[ssManStudio](../../Token/Other/ssManStudio_md.md)] to identify the top resource consuming queries and fix those that regressed due to the change of a plan choice, see [Query Store @Azure Blogs](https://azure.microsoft.com/blog/query-store-a-flight-data-recorder-for-your-database/).  
  
 When you identify a query with sub\-optimal performance, your action depends on the nature of the  problem.  
  
-   If the query was executed with multiple plans and the last plan is significantly worse than previous plan, you can use the plan forcing mechanism to force [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] to always use the optimal plan for future executions.  
  
     ![query-store-force-plan](../../Images/Image/ImageNotContaina/query-store-force-plan.png "query-store-force-plan")  
...
-   You may conclude that your query is missing an index for optimal execution. This information is surfaced within the query execution plan. Create the missing index and check the query performance using the Query Store.  
     ...
     ![query-store-show-plan](../../Images/Image/ImageNotContaina/query-store-show-plan.png "query-store-show-plan")  
  
     If you run your workload on [!INCLUDE[ssSDS](../../Token/Other/ssSDS_md.md)], sign up for [!INCLUDE[ssSDS](../../Token/Other/ssSDS_md.md)] Index Advisor to automatically receive index recommendations.  
...
```  
 ...
 When the actual state is read\-only, use the **readonly\_reason** column to determine the root cause. Typically you will find that Query Store transitioned to read\-only mode because the size quota was exceeded. In that case the **readonly\_reason** is set to 65536. For other reasons, see [sys.database_query_store_options &#40;Transact-SQL&#41;](../Topic/sys.database_query_store_options%20\(Transact-SQL\).md).  
  
 Consider the following steps to switch Query Store to read\-write mode and activate data collection:  
  
-   Increase the maximum storage size by using the **MAX\_STORAGE\_SIZE\_MB** option of **ALTER DATABASE**.  
...
-   Use the **Optimize for Ad Hoc Workloads** option if your workload contains many single use ad\-hoc batches with different query plans.  
    ...
    -   Compare the number of  distinct query\_hash values with the total number of entries in sys.query\_store\_query. If the ratio is close to 1 your ad\-hoc workload generates different queries.  
...
-   Apply FORCED PARAMETERIZATION, for the database or for a subset of queries if the number of different query plans is not large.  
    ...
    -   Configure FORCED PARAMETERIZATION for the database, if there are a small number of different query plans in your workload. \(When the ratio between the count of distinct query\_hash and the total number of entries in sys.query\_store\_query is much less than 1.\)  
...
```  
 ...
 For full list of reasons, refer to [sys.query_store_plan &#40;Transact-SQL&#41;](../Topic/sys.query_store_plan%20\(Transact-SQL\).md). You can also use the **query\_store\_plan\_forcing\_failed** XEvent to track troubleshoot plan forcing failures.  
  
##  <a name="Renaming"></a> Avoid Renaming Databases if you have Queries with Forced Plans  
 Execution plans reference objects using three\-part names \(`dbo.schema.object`\).   
