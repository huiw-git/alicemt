...
4.  In the **Events to log** section, select or clear the checkboxes to either enable or disable the following Analysis Services events:  
    ...
    |Event|Description|  
    |-----------|-----------------|  
    |**[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Connections**|[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Connection event is used to monitor [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] server connections that are made on behalf of a user.|  
    |**[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Load Data Usage**|[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Load Data Usage is used to monitor requests that load [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data into server memory. A load event is generated for [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data files loaded from a content database or from cache.|  
    |**[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Unload Data Usage**|[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Unload Data Usage is used to monitor requests for unloading a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data source after a period of inactivity. Caching a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data source to disk will be reported as an unload event.|  
    |**[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Query Usage**|[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Query Usage is used to monitor query processing times for data that is loaded in an [!INCLUDE[ssGeminiSrv](../../Token/Other/ssGeminiSrv_md.md)] instance.|  
...
##  <a name="configdb"></a> Set log file location  
 [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] usage data is initially stored in usage log files on the local server, and then moved at regular intervals to the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service application databases. The log file location is set in Central Administration. The default location is:  
...
##  <a name="jobs"></a> Configure the timer jobs used in usage data collection  
 [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] server health and usage data is moved to different locations in the usage data collection system through two timer jobs.:  
  
-   The “Microsoft SharePoint Foundation Usage Data Import” timer job moves [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] usage to the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service application database.  
  
-   The “[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Management Dashboard Processing timer job” the data to [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook that is the source of data for built\-in administrative reports.  
  
 If you need to refresh the administrative reports that appear in the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Management Dashboard more frequently, follow these steps.  
...
5.  In the Job Definitions list, click **[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Data Management Dashboard Processing Timer Job**.  
...
##  <a name="confighist"></a> Limit how long usage data history is stored  
 Usage data history is stored for events \(connections, load, unload, and on\-demand query processing\) and data refresh \(scheduled data processing\). Although usage data is collected through the SharePoint usage data collection system, the reporting data is moved to a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] application database and a reporting database for longer term storage. The usage data history setting controls how long usage data is retained in the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] application databases. The same limit is applied equally to all types of stored usage data in the same [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service application database.  
...
##  <a name="openconfig"></a> Open the Power Pivot Service Application page to access configuration settings  
 You must be a farm or service administrator to modify service application settings. If you defined multiple [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service applications in the farm, you must modify each one individually.  
  
1.  In SharePoint Central Administration, in **Application Management**, click **Manage service applications**.  
  
2.  Find the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Service application. You can identify a service application by its type. A [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service application type is **[!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Service Application**.  
  
3.  Click the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service application name. The [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Management Dashboard opens.  
  
4.  In **Actions**, click **Configure service application settings**. The [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] Service Application Settings page will open.  
  
##  <a name="defaultconfig"></a> The default configuration for Power Pivot usage data collection  
 Usage data collection for [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] service operations can be enabled with default settings to make it immediately available in applications that support the Analysis Services integration feature. The default settings include events that trigger usage data collection, limits on how long usage data is stored, and thresholds for categorizing query response times.  
