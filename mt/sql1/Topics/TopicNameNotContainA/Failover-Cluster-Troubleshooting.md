 ...
 **Issue 1:** [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service accounts are unable to contact a domain controller.  
 ...
 **Issue 2:** [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service account passwords are not identical on all cluster nodes, or the node does not restart a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service that has migrated from a failed node.  
  
 **Resolution 2:** Change the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service account passwords using [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Configuration Manager. If you do not, and you change the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service account passwords on one node, you must also change the passwords on all other nodes. [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Configuration Manager does this automatically.  
 ...
 **Resolution 2:** Disk drive letters for the cluster disks must be the same on both servers. If they are not, review your original installation of the operating system and [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Cluster Service \(MSCS\).  
  
### Problem: Failure of a SQL Server service causes failover  
 **Resolution:** To prevent the failure of specific services from causing the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] group to fail over, configure those services using Cluster Administrator in Windows, as follows:  
  
-   Clear the **Affect the Group** check box on the **Advanced** tab of the **Full Text Properties** dialog box. However, if [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] causes a failover, the full\-text search service restarts.  
  
### Problem: SQL Server does not start automatically  
 **Resolution:** Use Cluster Administrator in MSCS to automatically start a failover cluster. The [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service should be set to start manually; the Cluster Administrator should be configured in MSCS to start the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service. For more information, see [Managing Services](https://msdn.microsoft.com/library/ms178096\(v=sql.105\).aspx).  
 ...
 **Issue 3:** [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] is not connecting using Named Pipes.  
...
1.  Determine on which node the group containing the instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] is running by using the Cluster Administrator. For this example, it is **Node A**.  
  
2.  Start the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] service on that computer using **net start**. For more information about using **net start**, see [Starting SQL Server Manually](https://msdn.microsoft.com/library/ms191193\(v=sql.105\).aspx).  
  
3.  Start the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] SQL Server Configuration Manager on **Node A**. View the pipe name on which the server is listening. It should be similar to \\\\.\\$$\\VIRTSQL\\pipe\\sql\\query.  
 ...
 **Issue:** An orphan registry key in \[HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL.X\\Cluster\]  
...
## Using Extended Stored Procedures and COM Objects  
 When you use extended stored procedures with a failover clustering configuration, all extended stored procedures must be installed on a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]\-dependent cluster disk. Doing so ensures that when a node fails over, the extended stored procedures can still be used.  
