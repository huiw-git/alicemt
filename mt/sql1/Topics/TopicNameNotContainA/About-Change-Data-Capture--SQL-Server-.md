...
## Change Data Capture Validity Interval for a Capture Instance  
 ...
 The function [sys.fn_cdc_get_min_lsn](assetId:///bd49e28a-128b-4f6b-8545-6a2ec3f4afb3) is used to retrieve the current minimum LSN for a capture instance, while [sys.fn_cdc_get_max_lsn](assetId:///93f3a4c8-b91f-4ebb-8e96-9397bb3a1c43) is used to retrieve the current maximum LSN value. When querying for change data, if the specified LSN range does not lie within these two LSN values, the change data capture query functions will fail.  
...
> [!IMPORTANT]  
>  Both instances of the capture logic require [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Agent to be running for the process to execute.  
...
## Change Data Capture Agent Jobs  
 Two [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Agent jobs are typically associated with a change data capture enabled database: one that is used to populate the database change tables, and one that is responsible for change table cleanup. Both jobs consist of a single step that runs a [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] command. The [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] command that is invoked is a change data capture defined stored procedure that implements the logic of the job. The jobs are created when the first table of the database is enabled for change data capture. The Cleanup Job is always created. The capture job will only be created if there are no defined transactional publications for the database. The capture job is also created when both change data capture and transactional replication are enabled for a database, and the transactional logreader job is removed because the database no longer has defined publications.  
 ...
 Internally, change data capture agent jobs are created and dropped by using the stored procedures [sys.sp_cdc_add_job](assetId:///c4458738-ed25-40a6-8294-a26ca5a05bd9) and [sys.sp_cdc_drop_job](assetId:///e8265846-8051-4848-b28e-fac27c10bdeb), respectively. These stored procedures are also exposed so that administrators can control the creation and removal of these jobs.  
  
 An administrator has no explicit control over the default configuration of the change data capture agent jobs. The stored procedure [sys.sp_cdc_change_job](assetId:///ea918888-0fc5-4cc1-b301-26b2a9fbb20d) is provided to allow the default configuration parameters to be modified. In addition, the stored procedure [sys.sp_cdc_help_jobs](assetId:///9399b4bc-8293-408f-b3cb-f904e0657fb5) allows current configuration parameters to be viewed. Both the capture job and the cleanup job extract configuration parameters from the table msdb.dbo.cdc_jobs on startup. Any changes made to these values by using [sys.sp_cdc_change_job](assetId:///ea918888-0fc5-4cc1-b301-26b2a9fbb20d) will not take effect until the job is stopped and restarted.  
  
 Two additional stored procedures are provided to allow the change data capture agent jobs to be started and stopped: [sys.sp_cdc_start_job](assetId:///cf443a67-7705-4799-9f39-0e3a6a8a0708) and [sys.sp_cdc_stop_job](assetId:///421fc21c-c7a4-407c-8b31-359273b68c63).  
