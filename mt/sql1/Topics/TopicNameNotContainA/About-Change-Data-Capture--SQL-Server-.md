...
## Change Data Capture Validity Interval for a Database  
 ...
 At the high end, as the capture process commits each new batch of change data, new entries are added to **cdc.lsn\_time\_mapping** for each transaction that has change table entries. Within the mapping table, both a commit Log Sequence Number \(LSN\) and a transaction commit time \(columns start\_lsn and tran\_end\_time, respectively\) are retained. The maximum LSN value that is found in **cdc.lsn\_time\_mapping** represents the high water mark of the database validity window. Its corresponding commit time is used as the base from which retention based cleanup computes a new low water mark.  
...
## Change Data Capture Validity Interval for a Capture Instance  
 Although it is common for the database validity interval and the validity interval of individual capture instance to coincide, this is not always true. The validity interval of the capture instance starts when the capture process recognizes the capture instance and starts to log associated changes to its change table. As a result, if capture instances are created at different times, each will initially have a different low endpoint. The start\_lsn column of the result set that is returned by [sys.sp\_cdc\_help\_change\_data\_capture](../Topic/sys.sp_cdc_help_change_data_capture%20\(Transact-SQL\).md) shows the current low endpoint for each defined capture instance. When the cleanup process cleans up change table entries, it adjusts the start\_lsn values for all capture instances to reflect the new low water mark for available change data. Only those capture instances that have start\_lsn values that are currently less than the new low water mark are adjusted. Over time, if no new capture instances are created, the validity intervals for all individual instances will tend to coincide with the database validity interval.  
 ...
 The function [sys.fn\_cdc\_get\_min\_lsn](../Topic/sys.fn_cdc_get_min_lsn%20\(Transact-SQL\).md) is used to retrieve the current minimum LSN for a capture instance, while [sys.fn\_cdc\_get\_max\_lsn](../Topic/sys.fn_cdc_get_max_lsn%20\(Transact-SQL\).md) is used to retrieve the current maximum LSN value. When querying for change data, if the specified LSN range does not lie within these two LSN values, the change data capture query functions will fail.  
...
> [!IMPORTANT]  
>  Both instances of the capture logic require [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Agent to be running for the process to execute.  
...
## Change Data Capture Agent Jobs  
 ...
 Internally, change data capture agent jobs are created and dropped by using the stored procedures [sys.sp\_cdc\_add\_job](../Topic/sys.sp_cdc_add_job%20\(Transact-SQL\).md) and [sys.sp\_cdc\_drop\_job](../Topic/sys.sp_cdc_drop_job%20\(Transact-SQL\).md), respectively. These stored procedures are also exposed so that administrators can control the creation and removal of these jobs.  
  
 An administrator has no explicit control over the default configuration of the change data capture agent jobs. The stored procedure [sys.sp\_cdc\_change\_job](../Topic/sys.sp_cdc_change_job%20\(Transact-SQL\).md) is provided to allow the default configuration parameters to be modified. In addition, the stored procedure [sys.sp\_cdc\_help\_jobs](../Topic/sys.sp_cdc_help_jobs%20\(Transact-SQL\).md) allows current configuration parameters to be viewed. Both the capture job and the cleanup job extract configuration parameters from the table msdb.dbo.cdc\_jobs on startup. Any changes made to these values by using [sys.sp\_cdc\_change\_job](../Topic/sys.sp_cdc_change_job%20\(Transact-SQL\).md) will not take effect until the job is stopped and restarted.  
  
 Two additional stored procedures are provided to allow the change data capture agent jobs to be started and stopped: [sys.sp\_cdc\_start\_job](../Topic/sys.sp_cdc_start_job%20\(Transact-SQL\).md) and [sys.sp\_cdc\_stop\_job](../Topic/sys.sp_cdc_stop_job%20\(Transact-SQL\).md).  
