 ...
 **-if** *workload_file*  
 Specifies the path and name of the workload file to use as input for tuning. The file must be in one of these formats: .trc (SQL Server Profiler trace file), .sql (SQL file), or .log ([!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] trace file). Either one workload file or one workload table must be specified.  
 ...
 **-of** *output_script_file_name*  
 Specifies that **dta** writes the recommendation as a [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] script to the file name and destination specified.  
 ...
 **-S** *server_name*[ *\instance*]  
 Specifies the name of the computer and instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] to connect to. If no *server_name* is specified, **dta** connects to the default instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] on the local computer. This option is required when connecting to a named instance or when executing **dta** from a remote computer on the network.  
...
```  
dta –D tpcd1G –if tpcd_22.sql -B 3000 –of "d:result_dirscript1.sql" –A 60  
```  
