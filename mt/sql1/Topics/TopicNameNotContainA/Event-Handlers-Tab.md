...
---
# Event Handlers Tab
  Use the **Event Handlers** tab of [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer to build a control flow in an [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] package. An event handler runs in response to an event raised by the package or by a task or container in the package.  
 ...
 **Click here to create an \<event handler name\> for the executable \<executable name\>**  
 ...
 Create the control flow by dragging graphical objects that represent [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] tasks and containers from the **Toolbox** to the design surface of the **Event Handlers** tab, and then connecting the objects by using precedence constraints to define the sequence in which they run.  
