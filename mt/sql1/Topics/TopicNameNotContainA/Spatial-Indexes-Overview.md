...
[!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] supports spatial data and spatial indexes. A *spatial index* is a type of extended index that allows you to index a spatial column. A spatial column is a table column that contains data of a spatial data type, such as **geometry** or **geography**.  
  
> [!IMPORTANT]  
>  For a detailed description and examples of spatial features introduced in [!INCLUDE[ssSQL11](../../Topics/TopicNameContainA/tokens/ssSQL11_md.md)], including features that affect spatial indexes, download the white paper, [New Spatial Features in SQL Server 2012](http://go.microsoft.com/fwlink/?LinkId=226407).  
  
##  <a name="about"></a> About Spatial Indexes  
  
###  <a name="decompose"></a> Decomposing Indexed Space into a Grid Hierarchy  
 In [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)], spatial indexes are built using B-trees, which means that the indexes must represent the 2-dimensional spatial data in the linear order of B-trees. Therefore, before reading data into a spatial index, [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] implements a hierarchical uniform decomposition of space. The index-creation process *decomposes* the space into a four-level *grid hierarchy*. These levels are referred to as *level 1* (the top level), *level 2*, *level 3*, and *level 4*.  
 ...
 ![Four&#45;levels of recursive tessellation](../../Topics/TopicNameNotContainA/images/SpNdx_Recursive_Levels_Telescoped.gif "SpNdx_Recursive_Levels_Telescoped")  
...
>  The decomposition of space for a spatial index is independent of the unit of measurement that the application data uses.  
 ...
 ![Polygons and lines placed into a 4x4 level&#45;1 grid](../../Topics/TopicNameNotContainA/images/SpNdx_Level_1_Objects.gif "SpNdx_Level_1_Objects")  
...
#### Grid Density  
 ...
 The [CREATE SPATIAL INDEX](assetId:///ee6b9116-a7ff-463a-a9f0-b360804d8678)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement supports a GRIDS clause that enables you to specify different grid densities at different levels. The grid density for a given level is specified by using one of the following keywords.  
...
|HIGH|16X16|256|  
 ...
 In [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)], when the database compatibility level is set to 100 or lower, then the default is MEDIUM on all levels. When the database compatibility level is set to 110 or higher, then the default is an auto grid scheme.  
...
> [!NOTE]  
>  The grid densities of a spatial index are visible in the level_1_grid, level_2_grid, level_3_grid, and level_4_grid columns of the [sys.spatial_index_tessellations](assetId:///8b17a9a4-b57f-4220-8138-fc73581b1670) catalog view when the database compatibility level is set to 100 or lower. The **GEOMETRY_AUTO_GRID**/**GEOGRAPHY_AUTO_GRID** tessellation scheme options do not populate these columns. sys.spatial_index_tessellations catalog view has **NULL** values for these columns when the auto grid options are used.  
...
#### Covering Rule  
 ...
 ![Covering optimization](../../Topics/TopicNameNotContainA/images/SpNdx_Opt_Covering.gif "SpNdx_Opt_Covering")  
...
#### Cells-Per-Object Rule  
 ...
 By default, the cells-per-object limit is 16 cells per object, which provides a satisfactory trade-off between space and precision for most spatial indexes. However, the [CREATE SPATIAL INDEX](assetId:///ee6b9116-a7ff-463a-a9f0-b360804d8678)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement supports a CELLS_PER_OBJECT**=***n* clause that enables you to specify a cells-per-object limit between 1 and 8192, inclusive.  
  
> [!NOTE]  
>  The **cells_per_object** setting of a spatial index is visible in the [sys.spatial_index_tessellations](assetId:///8b17a9a4-b57f-4220-8138-fc73581b1670) catalog view.  
...
#### Deepest-Cell Rule  
 ...
 ![Deepest&#45;cell optimization](../../Topics/TopicNameNotContainA/images/SpNdx_Opt_Deepest_Cell.gif "SpNdx_Opt_Deepest_Cell")  
  
###  <a name="schemes"></a> Tessellation Schemes  
 The behavior of a spatial index depends partly on its *tessellation scheme*. The tessellation scheme is data-type specific. In [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)], spatial indexes support two tessellation schemes:  
...
> [!NOTE]  
>  The **tessellation_scheme** setting of a spatial index is visible in the [sys.spatial_index_tessellations](assetId:///8b17a9a4-b57f-4220-8138-fc73581b1670) catalog view.  
  
#### Geometry Grid Tessellation Scheme  
 GEOMETRY_AUTO_GRID tessellation is the default tessellation scheme for the **geometry** data type for [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] and later.  GEOMETRY_GRID tessellation is the only tessellation scheme available for geometry data types in [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)]. This section discusses aspects of geometry grid tessellation that are relevant to working with spatial indexes: supported methods and bounding boxes.  
  
> [!NOTE]  
>  You can explicitly specify this tessellation scheme by using the USING (GEOMETRY_AUTO_GRID/GEOMETRY_GRID) clause of the [CREATE SPATIAL INDEX](assetId:///ee6b9116-a7ff-463a-a9f0-b360804d8678)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement.  
  
##### The Bounding Box  
 Geometric data occupies a plane that can be infinite. In [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)], however, a spatial index requires a finite space. To establish a finite space for decomposition, the geometry grid tessellation scheme requires a rectangular *bounding box*. The bounding box is defined by four coordinates, **(***x-min***,***y-min***)** and **(***x-max***,***y-max***)**, which are stored as properties of the spatial index. These coordinates represent the following:  
...
> [!NOTE]  
>  These coordinates are specified by the BOUNDING_BOX clause of the [CREATE SPATIAL INDEX](assetId:///ee6b9116-a7ff-463a-a9f0-b360804d8678)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement.  
 ...
 ![Bounding box showing coordinates and cell 0.](../../Topics/TopicNameNotContainA/images/SpNdx_BB_4x4_Objects.gif "SpNdx_BB_4x4_Objects")  
...
> [!NOTE]  
>  The grid densities of a spatial index are visible in the bounding_box_xmin, bounding_box_ymin, bounding_box_xmax, and bounding_box_ymax columns of the [sys.spatial_index_tessellations](assetId:///8b17a9a4-b57f-4220-8138-fc73581b1670) catalog view.  
...
> [!NOTE]  
>  You can explicitly specify this tessellation scheme by using the USING (GEOGRAPHY_AUTO_GRID/GEOGRAPHY_GRID) clause of the [CREATE SPATIAL INDEX](assetId:///ee6b9116-a7ff-463a-a9f0-b360804d8678)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement.  
...
3.  Joins the flattened pyramids to form a non-Euclidean plane.  
 ...
 ![Projection of the ellipsoid onto a plane](../../Topics/TopicNameNotContainA/images/SpNdx_Geodetic_Projection.gif "SpNdx_Geodetic_Projection")  
 ...
 ![Level&#45;1 geography grid](../../Topics/TopicNameNotContainA/images/SpNdx_Geodetic_Level1Grid.gif "SpNdx_Geodetic_Level1Grid")  
...
-   *geometry1*.[STContains](assetId:///865ceca1-9200-45ed-a7d8-e286e2679fdc)(*geometry2*) = 1  
  
-   *geometry1*.[STDistance](assetId:///ac815bc7-5342-4cc4-af40-c80a1c4c8b68)(*geometry2*) < *number*  
  
-   *geometry1*.[STDistance](assetId:///ac815bc7-5342-4cc4-af40-c80a1c4c8b68)(*geometry2*) <= *number*  
  
-   *geometry1*.[STEquals](assetId:///808f0e25-9e68-4ba7-9329-07ec950698f3)(*geometry2*)= 1  
  
-   *geometry1*.[STIntersects](assetId:///7c18f5be-5a29-422e-8ca7-d8a5f38e03f5)(*geometry2*)= 1  
  
-   *geometry1.* [STOverlaps](assetId:///1813cba1-5780-456a-9489-6b40a79569b3) *(geometry2) = 1*  
  
-   *geometry1*.[STTouches](assetId:///af3650b4-26da-4600-9cc2-1be71dd76a14)(*geometry2*) = 1  
  
-   *geometry1*.[STWithin](assetId:///f845d28c-8029-4e2b-bcf0-71c52a592501)(*geometry2*)= 1  
...
-   *geography1*.[STIntersects](assetId:///c9db8b42-83c7-48c6-8963-fce54eb34c05)(*geography2*)= 1  
  
-   *geography1*.[STEquals](assetId:///0766ff37-0b9e-49bf-83c0-019f4354fe44)(*geography2*)= 1  
  
-   *geography1*.[STDistance](assetId:///063d8722-e019-4d3d-8fcf-dbf5325823e7)(*geography2*) < *number*  
