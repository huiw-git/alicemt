...
## Automatic Soft-NUMA  
 ...
 ![Soft&#45;NUMA Error Log](../../Topics/TopicNameNotContainA/images/Soft-NUMA-Error-Log.PNG "Soft-NUMA)  
  
## Manual Soft-NUMA  
 To can manually configure   
            [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] to use soft-NUMA by disabling automatic soft_NUMA and editing the registry to add a node configuration affinity mask. When using this method, the soft-NUMA mask can be stated as a binary, DWORD (hexadecimal or decimal), or QWORD (hexadecimal or decimal) registry entry. To configure more than the first 32 CPUs use QWORD or BINARY registry values. (QWORD values cannot be used prior to   
            [!INCLUDE[ssSQL11](../../Topics/TopicNameContainA/tokens/ssSQL11_md.md)].) After modifying the registry, you must restart the   
            [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] for the soft-NUMA configuration to take  effect.  
  
> [!TIP]  
>  CPUs are numbered starting with 0.  
  
 [!INCLUDE[ssNoteRegistry](../../Topics/TopicNameNotContainA/tokens/ssNoteRegistry_md.md)]  
 ...
 Consider the following example. A computer with eight CPUs does not have hardware NUMA. Three soft-NUMA nodes are configured.   
            [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] instance A is configured to use CPUs 0 through 3. A second instance of the   
            [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] is installed and configured to use CPUs 4 through 7. The example can be visually represented as:  
...
|Example for a <br />                                        [!INCLUDE[ssSQL15](../../Topics/TopicNameContainA/tokens/ssSQL15_md.md)] server with multiple K-Groups|Type|Value name|Value data|  
|-----------------------------------------------------------------------------------------------------------------|----------|----------------|----------------|  
