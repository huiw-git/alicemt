...
# Prerequisites, Restrictions, and Recommendations for Always On Availability Groups (SQL Server)
  This topic describes considerations for deploying [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)], including prerequisites, restrictions, and recommendations for host computers, Windows Server Failover Clustering \(WSFC\) clusters, server instances, and availability groups. For each of these components security considerations and required permissions, if any, are indicated.  
  
> [!IMPORTANT]  
>  Before you deploy [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)], we strongly recommend that you read every section of this topic.  
    
##  <a name="DotNetHotfixes"></a> .Net Hotfixes that Support Availability Groups  
 Depending on the [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] components and features you will use with [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)], you may need to install additional .Net hotfixes identified in the following table. The hotfixes can be installed in any order.  
  
||Dependent Feature|Hotfix|Link|  
|------|-----------------------|------------|----------|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|[!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)]|Hotfix for .Net 3.5 SP1 adds support to SQL Client for Always On features of Read\-intent, readonly, and multisubnetfailover. The hotfix needs to be installed on each [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] report server.|KB 2654347: [Hotfix for .Net 3.5 SP1 to add support for Always On features](http://go.microsoft.com/fwlink/?LinkId=242896)|  
...
###  <a name="SystemRequirements"></a> Checklist: Requirements \(Windows System\)  
 To support the [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] feature, ensure that every computer that is to participate in one or more availability groups meets the following fundamental requirements:  
  
||Requirement|Link|  
|------|-----------------|----------|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Ensure that the system is not a domain controller.|Availability groups are not supported on domain controllers.|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Ensure that each computer is running Windows Server 2012 or later versions.|[Hardware and Software Requirements for Installing SQL Server 2016](../../Topics/TopicNameNotContainA/Hardware-and-Software-Requirements-for-Installing-SQL-Server-2016.md)|  
...
-   **Dedicated network adapters:**  For best performance, use a dedicated network adapter \(network interface card\) for [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)].  
...
###  <a name="ThreadUsage"></a> Thread Usage by Availability Groups  
 [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] has the following requirements for worker threads:  
  
-   On an idle instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)], [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] uses 0 threads.  
...
-   The availability replicas hosted on a given server instance share a single thread pool.  
    ...
    -   If a given thread is idle for a while, it is released back into the general [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] thread pool. Normally, an inactive thread is released after ~15 seconds of inactivity. However, depending on the last activity, an idle thread might be retained longer.  
...
|Enabling [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)]|Requires membership in the **Administrator** group on the local computer and full control on the WSFC cluster.|  
...
|Enabling Availability Groups|[Enable and Disable Always On Availability Groups &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Enable-and-Disable-Always-On-Availability-Groups--SQL-Server-.md)|  
...
##  <a name="ClientConnSupport"></a> Client Connectivity Support  
 For information about [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] support for client connectivity, see [Always On Client Connectivity &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Always-On-Client-Connectivity--SQL-Server-.md).  
...
||Prerequisite|Link|  
|-|------------------|----------|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Ensure that each SQL Server failover cluster instance \(FCI\) possesses the required shared storage as per standard SQL Server failover cluster instance installation.||  
...
-   [Failover Clustering and Availability Groups &#40;SQL Server&#41;](../../Topics/TopicNameNotContainA/Failover-Clustering-and-Always-On-Availability-Groups--SQL-Server-.md)  
...
-   **Do not use the Failover Cluster Manager to manipulate availability groups:**  
    ...
    -   Do not use the Failover Cluster Manager to fail over availability groups. You must use [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] or [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)].  
...
-   Security is inherited from the Windows Server Failover Clustering \(WSFC\) cluster. WSFC provides two levels of user security at granularity of entire WSFC cluster APIs:  
    ...
    -   Full control  
         ...
         [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] needs full control, and enabling [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] on an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] gives it full control of the WSFC cluster \(through Service SID\).  
  
         You cannot directly add or remove security for a server instance in the WSFC Failover Cluster Manager. To manage WSFC security sessions, use the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Configuration Manager or the WMI equivalent from [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)].  
  
-   Each instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] must have permissions to access the registry, cluster, and soforth.  
  
-   We recommend that you use encryption for connections between server instances that host [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] availability replicas.  
...
||Requirements|Link|  
|-|------------------|----------|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Be a user database. System databases cannot belong to an availability group.||  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Reside on the instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] where you create the availability group and be accessible to the server instance.||  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Be a read\-write database. Read\-only databases cannot be added to an availability group.|[sys.databases](../Topic/sys.databases%20\(Transact-SQL\).md) \(**is\_read\_only** \= 0\)|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Be a multi\-user database.|[sys.databases](../Topic/sys.databases%20\(Transact-SQL\).md) \(**user\_access** \= 0\)|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Not use AUTO\_CLOSE.|[sys.databases](../Topic/sys.databases%20\(Transact-SQL\).md) \(**is\_auto\_close\_on** \= 0\)|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Use the full recovery model \(also known as, full recovery mode\).|[sys.databases](../Topic/sys.databases%20\(Transact-SQL\).md) \(**recovery\_model** \= 1\)|  
...
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Not belong to any existing availability group.|[sys.databases](../Topic/sys.databases%20\(Transact-SQL\).md) \(**group\_database\_id** \= NULL\)|  
|![Checkbox](../../Images/Image/ImageNotContaina/CheckboxEmptyCenterXtraSpaceTopAndRight.gif "CheckboxEmptyCenterXtraSpaceTopAndRight")|Not be configured for database mirroring.|[sys.database\_mirroring](../Topic/sys.database_mirroring%20\(Transact-SQL\).md) \(If the database does not participate in mirroring, all columns prefixed with "mirroring\_" are NULL.\)|  
...
> [!NOTE]  
>  [!INCLUDE[ssHADR](../../Token/Other/ssHADR_md.md)] works with any supported database compatibility level.  
...
-   If the file path \(including the drive letter\) of a secondary database differs from the path of the corresponding primary database, the following restrictions apply:  
    ...
    -   **[!INCLUDE[ssAoNewAgWiz](../../Token/Other/ssAoNewAgWiz_md.md)]\/[!INCLUDE[ssAoAddDbWiz](../../Token/Other/ssAoAddDbWiz_md.md)]:**  The **Full** option is not supported \(on the[Select Initial Data Synchronization Page](../../Topics/TopicNameNotContainA/Select-Initial-Data-Synchronization-Page--Always-On-Availability-Group-Wizards-.md) page\),  
  
    -   **RESTORE WITH MOVE:**  To create the secondary databases, the database files must be RESTORED WITH MOVE on each instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] that hosts a secondary replica.  
