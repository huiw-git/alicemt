...
---
# Row-Level Security
![Row level security graphic](../../Topics/TopicNameNotContainA/images/Row-level-security-graphic.png "Row)  
 ...
 Implement RLS by using the [CREATE SECURITY POLICY](assetId:///d6ab70ee-0fa2-469c-96f6-a3c16d673bc8)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement, and predicates created as [inline table valued functions](../../Topics/TopicNameNotContainA/Create-User-defined-Functions--Database-Engine-.md).  
  
||  
|-|  
|**Applies to**: [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] ([!INCLUDE[ssSQL15](../../Topics/TopicNameContainA/tokens/ssSQL15_md.md)] through [current version](http://go.microsoft.com/fwlink/p/?LinkId=299658)), [!INCLUDE[sqldbesa](../../Topics/TopicNameNotContainA/tokens/sqldbesa_md.md)] ([Get it](http://azure.micosoft.com/documentation/articles/sql-database-preview-whats-new/?WT.mc_id=TSQL_GetItTag)).|  
...
-   Avoid using excessive table joins in predicate functions to maximize performance.  
 ...
 Avoid predicate logic that depends on session-specific [SET options](assetId:///f7e107f8-0fcf-408b-b30f-da2323eeb714): While unlikely to be used in practical applications, predicate functions whose logic depends on certain session-specific **SET** options can leak information if users are able to execute arbitrary queries. For example, a predicate function that implicitly converts a string to **datetime** could filter different rows based on the **SET DATEFORMAT** option for the current session. In general, predicate functions should abide by the following rules:  
  
-   Predicate functions should not implicitly convert character strings to **date**, **smalldatetime**, **datetime**, **datetime2**, or **datetimeoffset**, or vice versa, because these conversions are affected by the [SET DATEFORMAT (Transact-SQL)](assetId:///da217878-7ec4-477e-aa13-604073c948f8) and [SET LANGUAGE (Transact-SQL)](assetId:///0ec0e5cf-e115-4be9-a0db-e65837d6fa45) options. Instead, use the **CONVERT** function and explicitly specify the style parameter.  
  
-   Predicate functions should not rely on the value of the first day of the week, because this value is affected by the [SET DATEFIRST (Transact-SQL)](assetId:///6b0d0e52-8ac1-4f88-b091-f98d6fb8574a) option.  
  
-   Predicate functions should not rely on arithmetic or aggregation expressions returning **NULL** in case of error (such as overflow or divide-by-zero), because this behavior is affected by the [SET ANSI_WARNINGS (Transact-SQL)](assetId:///f82aaab0-334f-427b-89b0-de4af596b4fa), [SET NUMERIC_ROUNDABORT (Transact-SQL)](assetId:///d20e74f1-b8da-466c-b180-9d8a8b851a77), and [SET ARITHABORT (Transact-SQL)](assetId:///f938a666-fdd1-4233-b97f-719f27b1a0e6) options.  
  
-   Predicate functions should not compare concatenated strings with **NULL**, because this behavior is affected by the [SET CONCAT_NULL_YIELDS_NULL (Transact-SQL)](assetId:///3091b71c-6518-4eb4-88ab-acae49102bc5) option.  
...
##  <a name="Limitations"></a> Cross-Feature Compatibility  
 In general, row-level security will work as expected across features. However, there are a few exceptions. This section documents several notes and caveats for using row-level security with certain other features of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)].  
...
###  <a name="MidTier"></a> B. Scenario for users who connect to the database through a middle-tier application  
 This example shows how a middle-tier application can implement connection filtering, where application users (or tenants) share the same [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] user (the application). The application sets the current application user ID in [SESSION_CONTEXT (Transact-SQL)](assetId:///b6bdbc54-331a-43cc-ab3d-3872d6a12100) after connecting to the database, and then security policies transparently filter rows that shouldn't be visible to this ID, and also block the user from inserting rows for the wrong user ID. No other app changes are necessary .  
...
## See Also  
 [CREATE SECURITY POLICY (Transact-SQL)](assetId:///d6ab70ee-0fa2-469c-96f6-a3c16d673bc8)   
 [ALTER SECURITY POLICY (Transact-SQL)](assetId:///a8efc37e-113d-489c-babc-b914fea2c316)   
 [DROP SECURITY POLICY (Transact-SQL)](assetId:///5bd3393d-2fa5-4db0-a69a-a1a72d638e9d)   
 [CREATE FUNCTION (Transact-SQL)](assetId:///864b393f-225f-4895-8c8d-4db59ea60032)   
 [SESSION_CONTEXT (Transact-SQL)](assetId:///b6bdbc54-331a-43cc-ab3d-3872d6a12100)   
 [sp_set_session_context (Transact-SQL)](assetId:///7a3a3b2a-1408-4767-a376-c690e3c1fc5b)   
 [sys.security_policies (Transact-SQL)](assetId:///35362f5b-e601-4049-9e1d-c5307e823831)   
 [sys.security_predicates (Transact-SQL)](assetId:///c7a2f28c-98da-463d-8b8a-8e5619e2c6a6)   
