...
# Comparing Tabular and Multidimensional Solutions (SSAS)
  [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] provides several approaches for creating a business intelligence semantic model:  Multidimensional, Tabular, and Power Pivot.  
 ...
 Tabular and Multidimensional solutions are built using [!INCLUDE[ssBIDevStudio](../../Token/Other/ssBIDevStudio_md.md)] and are intended for corporate BI projects that run on a standalone [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance. Both solutions yield high performance analytical databases that integrate easily with BI clients. Yet each solution differs in how they are created, used, and deployed. The bulk of this topic compares these two types so that you can identify the right approach for you.  
...
-   [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks use DAX for calculations, and DAX or MDX for queries.  
...
-   [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks are secured at the file level, using SharePoint permissions.  
...
-   Multidimensional model databases can use dimension and cell\-level security, using role\-based permissions in Analysis Services.  
 ...
 [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks can be restored to a Tabular mode server. Once the file is restored, it is decoupled from SharePoint, allowing you to use all Tabular modeling features, including row\-level security.  
...
##  <a name="bkmk_client"></a> Client Application Support  
 ...
 [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)], a Reporting Services authoring tool that runs in SharePoint, is available on a report server that is deployed in a SharePoint 2010 farm. The only type of data source that can be used with this report is an Analysis Services tabular model database or a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook. This means that you must have a tabular mode server or a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for SharePoint server to host the data source used by this type of report. You cannot use a multidimensional model as a data source for a [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)] report. You must create a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] BI Semantic Model connection or a Reporting Services shared data source to use as the data source for a [!INCLUDE[ssCrescent](../../Token/Other/ssCrescent_md.md)] report.  
  
 Report Builder and Report Designer can use any Analysis Services database, including [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks that are hosted on [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for SharePoint.  
  
 Excel PivotTable reports are supported by all Analysis Services databases. Excel functionality is the same whether you use a tabular .database, multidimensional database, or [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbook, although Writeback is only supported for multidimensional databases.  
  
 PerformancePoint dashboards can connect to all Analysis Services databases, including [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks. For more information, see [Create Data Connections \(PerformancePoint Services\)](http://go.microsoft.com/fwlink/?linkdID=218155).  
...
|1 \- [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for SharePoint|For [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data access, Analysis Services is an internal component of a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for SharePoint installation. Analysis Services is installed in deployment mode 1 and used exclusively by [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] services in a SharePoint environment. For more information, see [Power Pivot for SharePoint 2010 Installation](assetId:///8d47dde7-c941-4280-a934-e2fe3f9a938f).|  
...
##  <a name="bkmk_sharePoint"></a> SharePoint Requirements  
 SQL Server integrates with SharePoint by adding support for [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data access and tabular data access. Investment in SharePoint and SQL Server integration grows when you maximize the number of features used from each product. If you have SharePoint, you can install SQL Server [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for SharePoint to enable [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data access and get the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] .bism connection files used to access tabular databases running on an external Analysis Services instance on a network server.  
  
 Power View reporting, which uses [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] and tabular databases as a data source, is both a SharePoint feature provided by SQL Server and a built\-in feature of Excel. Although the tabular databases run on an Analysis Services instance outside of SharePoint, that data is consumed by Power View reports that run in SharePoint.  
  
 If you do not use SharePoint, you can still use [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for Excel to create [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks but you will not have a cohesive data visualization experience. Each person who uses the workbook must download and view each workbook in Excel using the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] for Excel add\-in to get data interaction and exploration using slicers, filters, and pivots. Otherwise, workbook visualization is limited to static data as it appears when you open the workbook.  
...
##  <a name="bkmk_ext"></a> Programmability and Extensibility Support  
 Although there is developer support for Power BI, there is no developer support for [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks. If you are using [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] workbooks, you must use the built\-in client and server applications as part of your solution. Excel programming and SharePoint programming are the only options.  
...
-   Build a [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] model using the [Power Pivot for Excel Tutorial](http://go.microsoft.com/fwlink/?LinkId=251135).  
