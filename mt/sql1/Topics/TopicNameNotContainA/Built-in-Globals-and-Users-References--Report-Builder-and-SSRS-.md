...
-   This expression provides the name of the report and the time it was run. The time is formatted with the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[dnprdnshort](../../Token/Other/dnprdnshort_md.md)] formatting string for short date:  
...
|**UserID**|**String**|The ID of the user running the report. If you are using Windows Authentication, this value is the domain account of the current user. The value is determined by the [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] security extension, which can use Windows Authentication or custom authentication.|  
  
 For more information about supporting multiple languages in a report, see "Solution Design Considerations for Multi\-Lingual or Global Deployments" in the [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] documentation in [SQL Server Books Online](http://go.microsoft.com/fwlink/?LinkId=120955).  
