---
title: Power Pivot Configuration using Windows PowerShell
ms.custom: na
...
# Power Pivot Configuration using Windows PowerShell
  [!INCLUDE[ssCurrent](../../Topics/TopicNameContainA/includes/ssCurrent_md.md)] includes Windows PowerShell cmdlets that you can use to configure an installation of [!INCLUDE[ssGeminiShort](../../Topics/TopicNameNotContainA/includes/ssGeminiShort_md.md)]. To fully configure an installation with PowerShell requires the use of both SharePoint cmdlets and [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] for SharePoint cmdlets. A majority of configuration can be completed using one of the [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] tools. For more information on the tools, see [Power Pivot Configuration Tools](../../Topics/TopicNameNotContainA/Power-Pivot-Configuration-Tools.md).  
...
## View a list of the Power Pivot Cmdlets on a Server  
 To see content and examples of the [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] cmdlets, see [PowerShell Reference for Power Pivot for SharePoint](../../Topics/TopicNameNotContainA/PowerShell-Reference-for-Power-Pivot-for-SharePoint.md).  
  
 To use PowerShell to view a list of the [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] cmdlets:  
...
2.  Enter the following command:  
    ...
    ```  
     ...
     You should see a list of cmdlets that include [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] in the cmdlet name. For example `Get-PowerPivotServiceApplication`. The number of cmdlets available depends on the version of Analysis Services you are using.  
    ...
    -   17 cmdlets with SQL Server 2012 Analysis Services server configured in SharePoint mode, and SharePoint 2010.  
     ...
     If no commands are returned in the list or you see an error message similar to “`get-help could not find *powerpivot* in a help file in this session.`“, see the next section in this topic for instructions on how to enable the [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] cmdlets on the server.  
...
## Enable Power Pivot Cmdlets on a Server  
 [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] cmdlets are available after you install [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] for SharePoint and deploy the farm solution. The solutions are deployed when you ran the [!INCLUDE[ssGemini](../../Topics/TopicNameContainA/includes/ssGemini_md.md)] Configuration tool. Follow these steps to enable the use of cmdlets:  
