...
[!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Remote BLOB Store (RBS) is an optional add-on component that lets database administrators store binary large objects in commodity storage solutions instead of directly on the main database server.  
  
 RBS is included on the [!INCLUDE[ssCurrent](../../Topics/TopicNameContainA/tokens/ssCurrent_md.md)] installation media, but is not installed by the [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Setup program.  
...
### Custom providers  
 When you use a custom provider to store BLOBs outside of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)], make sure that you protect the stored BLOBs with permissions and encryption options that are appropriate to the storage medium used by the custom provider.  
...
-   RBS 2016 uses an **AES_128** symmetric key. [!INCLUDE[ssSQL15](../../Topics/TopicNameContainA/tokens/ssSQL15_md.md)] does not allow the creation of new **TRIPLE_DES** keys except for backwards compatibility reasons. For more information, see [CREATE SYMMETRIC KEY (Transact-SQL)](assetId:///b5d23572-b79d-4cf1-9eef-d648fa3b1358).  
  
-   RBS 2014 and prior versions use a credential store which holds secrets encrypted using the **TRIPLE_DES** symmetric key algorithm which is outdated. If you are currently using **TRIPLE_DES**[!INCLUDE[msCoName](../../Topics/TopicNameContainA/tokens/msCoName_md.md)] recommends that you enhance your security by following the steps in this topic to rotate your key to a stronger encryption method.  
  
 You can determine the RBS credential store symmetric key properties by executing the following [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement in the RBS database:   
 ...
 **RBS documentation**  
 The RBS documentation is included in the Windows installer package. If you want to review the RBS documentation without installing RBS, you can view the [!INCLUDE[ssKilimanjaro](../../Topics/TopicNameContainA/tokens/ssKilimanjaro_md.md)] version of the documentation [online in the MSDN Library](http://go.microsoft.com/fwlink/?LinkId=210192).  
