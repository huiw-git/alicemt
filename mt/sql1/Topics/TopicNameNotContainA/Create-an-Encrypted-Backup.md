...
### Backup to Windows Azure Storage with Encryption  
 If you are creating a backup to Windows Azure storage using the **SQL Server Backup to URL** option, the encryption steps are the same, but you must use URL as the destination and a SQL Credential to authenticate to the Windows Azure storage. If you want to configure [!INCLUDE[ss_smartbackup](../../Token/Other/ss_smartbackup_md.md)] with encryption options, see [Enable SQL Server Managed Backup to Microsoft Azure](../../Topics/TopicNameNotContainA/Enable-SQL-Server-Managed-Backup-to-Microsoft-Azure.md).  
