...
The [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] debugger enables you to control which [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements are run in a [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] Query Editor window. You can pause the debugger on individual statements and then view the state of the code elements at that point.  
  
## Breakpoints  
 A breakpoint signals the debugger to pause execution on a specific [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement. For more information about breakpoints, see Using Transact-SQL Breakpoints.  
  
## Controlling Statement Execution  
 In the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] debugger, you can specify the following options for executing from the current statement in [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] code:  
  
-   Run to the next breakpoint.  
  
-   Step into the next statement.  
  
     If the next statement invokes a [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] stored procedure, function, or trigger, the debugger displays a new Query Editor window that contains the code of the module. The window is in debug mode, and execution pauses on the first statement in the module. You can then move through the module code, for example, by setting breakpoints or stepping through the code.  
...
-   Run from the current location to the current location of the pointer, and ignore all breakpoints.  
 ...
 The following table lists the various ways in which you can control how statements execute in the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] debugger.  
