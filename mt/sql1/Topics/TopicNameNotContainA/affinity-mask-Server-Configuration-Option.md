...
> [!NOTE]  
>  [!INCLUDE[ssNoteDepFutureDontUse](../../Token/Appliesto/ssNoteDepFutureDontUse_md.md)] Use [ALTER SERVER CONFIGURATION &#40;Transact-SQL&#41;](../Topic/ALTER%20SERVER%20CONFIGURATION%20\(Transact-SQL\).md) instead.  
...
>  Affinity support for servers with 33 to 64 processors is only available on 64\-bit operating systems.  
 ...
 The affinity mask option, which existed in earlier releases of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)], dynamically controls CPU affinity.  
  
 In [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)], the affinity mask option can be configured without requiring a restart of the instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. When you are using sp\_configure, you must run either RECONFIGURE or RECONFIGURE WITH OVERRIDE after setting a configuration option. When you are using [!INCLUDE[ssExpress](../../Token/Other/ssExpress_md.md)], changing the affinity mask option does require a restart.  
  
 Changes to the affinity masks occur dynamically, allowing for on\-demand startup and shutdown of the CPU schedulers that bind process threads within [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. This can occur as conditions change on the server. For example, if a new instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] is added to the server, it may be necessary to make adjustments to the affinity mask option to redistribute processor load.  
  
 Modifications to the affinity bitmasks require [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] to enable a new CPU scheduler and disable the existing CPU scheduler. New batches can then be processed on the new or remaining schedulers.  
  
 To start a new CPU scheduler, [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] creates a new scheduler and adds it to the list of its standard schedulers. The new scheduler is considered only for the new incoming batches. Current batches continue to run on the same scheduler. The workers migrate to the new scheduler as they free up, or as new workers are created.  
...
|255|11111111|0, 1, 2, 3, 4, 5, 6, and 7|  
 ...
 The affinity mask option is an advanced option. If you are using the sp\_configure system stored procedure to change the setting, you can change **affinity mask** only when **show advanced options** is set to 1. After executing the [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] RECONFIGURE command, the new setting takes effect immediately without requiring a restart of the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] instance.  
...
## Licensing Issues  
 Dynamic affinity is tightly constrained by CPU licensing. [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] does not allow any configuration of affinity mask options that violates the licensing policy.  
  
### Startup  
 If a specified affinity mask violates the licensing policy during [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] startup or during database attach, the engine layer will complete the startup process or database attach\/restore operation, and then it will reset the sp\_configure run value for the affinity mask to zero, issuing an error message to the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] error log.  
  
### Reconfigure  
 If a specified affinity mask violates the licensing policy when running [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] RECONFIGURE command, an error message is reported to the client session and to the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] error log, requiring the database administrator to reconfigure the affinity mask. No RECONFIGURE WITH OVERRIDE command is accepted in this case.  
