...
## Versions of the CE  
 ...
 In 1998, a major update of the CE was part of Microsoft SQL Server 7.0, for which the compatibility level was 70. Subsequent updates came with [!INCLUDE[ssSQL14](../../Token/Other/ssSQL14_md.md)] and [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] 2016, meaning compatibility levels 120 and 130. The CE updates for levels 120 and 130 incorporate assumptions and algorithms that work well on modern data warehousing workloads and on OLTP \(online transaction processing\).  
 ...
 **Query store:**Starting with [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] 2016 the query store is a handy tool for examining the performance of your queries.  In SQL Server Management Studio \(SSMS.exe\), in the **Object Explorer** under your database node, a **Query Store** node is displayed  when the query store is ON.  
 ...
 *Tip:* We recommend that each month you install the latest release of [\(SSMS.exe\)](http://msdn.microsoft.com/library/mt238290.aspx).  
 ...
 Another option for tracking the cardinality predictions of the CE is to use the extended event named **query\_optimizer\_estimate\_cardinality**.  The following T\-SQL code sample runs on [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. It writes a .xel file to C:\\Temp\\ \(although you can change the path\). When you open the .xel file in SSMS, its detailed information is displayed in a user friendly manner.  
...
1.  Open SSMS. Ensure your  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]database is set to the highest available compatibility level.  
  
2.  Perform the following preliminary steps:  
  
    1.  Open SSMS.  
  
    2.  Run the T\-SQL to ensure that your  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database is set to the highest available compatibility level.  
  
    3.  Ensure that your database has its LEGACY\_CARDINALITY\_ESTIMATION configuration turned OFF.  
...
- Execute **sp\_query\_store\_force\_plan**.  
