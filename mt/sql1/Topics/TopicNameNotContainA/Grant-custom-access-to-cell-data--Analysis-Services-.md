...
# Grant custom access to cell data (Analysis Services)
 ...
 ![Pivottable showing allowed and denied cells](../../Images/Image/ImageNotContaina/SSAS_permsCellsAllowed.png "SSAS_permsCellsAllowed")  
...
1.  In [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] connect to the instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)], select a database, open the **Roles** folder, and then click a database role \(or create a new database role\). Membership should already be specified, and the role should have **Read** access to the cube. See [Grant cube or model permissions &#40;Analysis Services&#41;](../../Topics/TopicNameNotContainA/Grant-cube-or-model-permissions--Analysis-Services-.md) if you need help with this step.  
  
2.  In **Cell Data**, check the cube selection to be sure you have chosen the right one, and then select **Enable read permissions**.  
  
     If you select just this check box, and do not provide an MDX expression, the effect is the same as denying access to all cells in the cube. This is because the default allowed set is an empty set whenever [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] resolves a subset of cube cells.  
...
```  
 ...
 ![Excel columns showing cells as not-available](../../Images/Image/ImageNotContaina/SSAS_permsCellsHideMeasure.png "SSAS_permsCellsHideMeasure")  
...
```  
 ...
 ![Excel table with available and unavailable cellls](../../Images/Image/ImageNotContaina/ssas_permsCalculatedCells.png "ssas_permsCalculatedCells")  
