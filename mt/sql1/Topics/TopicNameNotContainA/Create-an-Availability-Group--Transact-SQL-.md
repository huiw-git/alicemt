...
This topic describes how to use [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] to create and configure an availability group on instances of [!INCLUDE[ssCurrent](../../Topics/TopicNameContainA/tokens/ssCurrent_md.md)] on which the [!INCLUDE[ssHADR](../../Topics/TopicNameContainA/tokens/ssHADR_md.md)] feature is enabled. An *availability group* defines a set of user databases that will fail over as a single unit and a set of failover partners, known as *availability replicas*, that support failover.  
...
-   Before creating an availability group, verify that the instances of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] that host availability replicas reside on different Windows Server Failover Clustering (WSFC) node within the same WSFC failover cluster. Also, verify that each of the server instance meets all other [!INCLUDE[ssHADR](../../Topics/TopicNameContainA/tokens/ssHADR_md.md)] prerequisites. For more information, we strongly recommend that you read [Prerequisites, Restrictions, and Recommendations for Always On Availability Groups (SQL Server)](../../Topics/TopicNameNotContainA/Prerequisites--Restrictions--and-Recommendations-for-Always-On-Availability-Groups--SQL-Server-.md).  
...
###  <a name="SummaryTsqlStatements"></a> Summary of Tasks and Corresponding Transact-SQL Statements  
 The following table lists the basic tasks involved in creating and configuring an availability group and indicates which [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements to use for these tasks. The [!INCLUDE[ssHADR](../../Topics/TopicNameContainA/tokens/ssHADR_md.md)] tasks must be performed in the sequence in which they are presented in the table.  
  
|Task|Transact-SQL Statement(s)|Where to Perform Task**\***|  
|----------|----------------------------------|---------------------------------|  
|Create database mirroring endpoint (once per [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] instance)|[CREATE ENDPOINT](assetId:///6405e7ec-0b5b-4afd-9792-1bfa5a2491f6) *endpointName* … FOR DATABASE_MIRRORING|Execute on each server instance that lacks database mirroring endpoint.|  
...
> [!NOTE]  
>  For a sample configuration procedure containing code examples of each these [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements, see [Example: Configuring an Availability Group that Uses Windows Authentication](#ExampleConfigAGWinAuth).  
  
1.  Connect to the server instance that is to host the primary replica.  
  
2.  Create the availability group by using the [CREATE AVAILABILITY GROUP](assetId:///a3d55df7-b4e4-43f3-a14b-056cba36ab98)[!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement.  
...
##  <a name="ExampleConfigAGWinAuth"></a> Example: Configuring an Availability Group that Uses Windows Authentication  
 This example creates a sample [!INCLUDE[ssHADR](../../Topics/TopicNameContainA/tokens/ssHADR_md.md)] configuration procedure that uses [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] to set up database mirroring endpoints that use Windows Authentication and to create and configure an availability group and its secondary databases.  
...
-   The server instances must support [!INCLUDE[ssHADR](../../Topics/TopicNameContainA/tokens/ssHADR_md.md)]. For more information, see [Prerequisites, Restrictions, and Recommendations for Always On Availability Groups (SQL Server)](../../Topics/TopicNameNotContainA/Prerequisites--Restrictions--and-Recommendations-for-Always-On-Availability-Groups--SQL-Server-.md).  
...
-   Two sample databases, *MyDb1* and *MyDb2*, must exist on the server instance that will host the primary replica. The following code examples create and configure these two databases and create a full backup of each. Execute these code examples on the server instance on which you intend to create the sample availability group. This server instance will host the initial primary replica of the sample availability group.  
    ...
    1.  The following [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] example creates these databases and alters them to use the full recovery model:  
...
|Initial role|System|Host [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] Instance|  
|------------------|------------|---------------------------------------------|  
...
3.  > [!NOTE]  
    >  If the service accounts of the server instances that are to host your availability replicas run under the same domain account this step is unnecessary. Skip it and go directly to the next step.  
     ...
     The following code example shows the [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements for creating a login and granting it permission on an endpoint. The domain account of the remote server instance is represented here as *domain_name*\\*user_name*.  
...
4.  On the server instance where the user databases reside, create the availability group.  
    ...
    ```  
     ...
     For additional [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] code examples of creating an availability group, see [CREATE AVAILABILITY GROUP (Transact-SQL)](assetId:///a3d55df7-b4e4-43f3-a14b-056cba36ab98).  
...
|TCP://*COMPUTER01.Adventure-Works.com*:*7022*|Endpoint URL of the AgHostInstance instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] on COMPUTER01.|  
|TCP://*COMPUTER02.Adventure-Works.com*:*5022*|Endpoint URL of the default instance of [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] on COMPUTER02.|  
