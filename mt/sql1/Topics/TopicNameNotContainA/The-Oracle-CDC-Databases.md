...
|cdc\_update\_state\_interval|10|1|120|False|The size \(in Kbytes\) of memory chunks allocated for a transaction \(a transaction can allocate more than one chunk\). See the memory\_limit column in [cdc.xdbcdc_config](../../Topics/TopicNameNotContainA/The-Oracle-CDC-Databases.md#BKMK_cdcxdbcdc_config) table.|  
...
|sql\_reconnect\_interval|2|2|3600|False|The amount of time \(in seconds\) to wait before reconnecting to [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. This interval is used in addition to [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] client’s connect timeout.|  
