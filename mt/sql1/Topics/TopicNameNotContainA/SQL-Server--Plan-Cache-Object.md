...
The **Plan Cache** object provides counters to monitor how [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] uses memory to store objects such as stored procedures, ad hoc and prepared [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements, and triggers. Multiple instances of the **Plan Cache** object can be monitored at the same time, with each instance representing a different type of plan to monitor.  
...
|**Sql Plans**|Query plans produced from an ad hoc [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] query, including auto-parameterized queries, or from [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements prepared using **sp_prepare** or **sp_cursorprepare**. [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] caches the plans for ad hoc [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements for later reuse if the identical [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statement is later executed. User-parameterized queries (even if not explicitly prepared) are also monitored as Prepared SQL Plans.|  
