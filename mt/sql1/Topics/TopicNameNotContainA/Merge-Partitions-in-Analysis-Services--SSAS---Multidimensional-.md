...
# Merge Partitions in Analysis Services (SSAS - Multidimensional)
  You can merge partitions in an existing [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database to consolidate fact data from multiple partitions of the same measure group.  
...
-   They share the same string store compatibility level \(applies to partitioned distinct count measure groups only\).  
 ...
 Remote partitions can be merged only with other remote partitions that are defined with the same remote instance of [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)].  
...
##  <a name="bkmk_partitionsXMLA"></a> How to merge partitions using XMLA  
 See this topic for information, [Merging Partitions &#40;XMLA&#41;](../Topic/Merging%20Partitions%20\(XMLA\).md).  
