...
|**Destination table name \+ primary key name**|A logical relationship to any table in which a column name matches the selected table name concatenated with the name of the primary key column for the selected table, in that order. Non\-alphanumeric characters within the concatenation are ignored \(for example, "Product ID", "Product\_ID" and "ProductID" all match\).|  
