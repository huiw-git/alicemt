...
The **CursorImplicitConversion** event class describes cursor-implicit conversion events that occur in application programming interfaces (APIs) or [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] cursors. Cursor implicit conversion events occur when the [!INCLUDE[ssDEnoversion](../../Topics/TopicNameContainA/tokens/ssDEnoversion_md.md)] executes a Transact-SQL statement that is not supported by server cursors of the type requested. The [!INCLUDE[ssDE](../../Topics/TopicNameContainA/tokens/ssDE_md.md)] returns an error that indicates the cursor type has changed.  
