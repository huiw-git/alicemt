...
##  <a name="Benefits"></a> User-defined Function Benefits  
 The benefits of using user-defined functions in [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] are:  
...
-   They allow faster execution.  
     ...
     Similar to stored procedures, [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] user-defined functions reduce the compilation cost of [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] code by caching the plans and reusing them for repeated executions. This means the user-defined function does not need to be reparsed and reoptimized with each use resulting in much faster execution times.  
  
     CLR functions offer significant performance advantage over [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] functions for computational tasks, string manipulation, and business logic. [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] functions are better suited for data-access intensive logic.  
...
> [!NOTE]  
>  [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] user-defined functions in queries can only be executed on a single thread (serial execution plan).  
  
##  <a name="FunctionTypes"></a> Types of Functions  
 Scalar Function  
 User-defined scalar functions return a single data value of the type defined in the RETURNS clause. For an inline scalar function, there is no function body; the scalar value is the result of a single statement. For a multistatement scalar function, the function body, defined in a BEGIN...END block, contains a series of [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] statements that return the single value. The return type can be any data type except **text**, **ntext**, **image**, **cursor**, and **timestamp**.  
 ...
 System Functions  
 [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] provides many system functions that you can use to perform a variety of operations. They cannot be modified. For more information, see [Built-in Functions (Transact-SQL)](assetId:///17186213-5ab5-40b0-b470-b660af1ec44c), [System Stored Functions (Transact-SQL)](assetId:///8ad860e7-e054-4186-80a2-03977e42531d), and [Dynamic Management Views and Functions (Transact-SQL)](assetId:///cf893ecb-0bf6-4cbf-ac00-8a1099e405b1).  
  
##  <a name="Guidelines"></a> Guidelines  
 [!INCLUDE[tsql](../../Topics/TopicNameContainA/tokens/tsql_md.md)] errors that cause a statement to be canceled and continue with the next statement in the module (such as triggers or stored procedures) are treated differently inside a function. In functions, such errors cause the execution of the function to stop. This in turn causes the statement that invoked the function to be canceled.  
...
> [!NOTE]  
>  If a CREATE FUNCTION statement produces side effects against resources that do not exist when the CREATE FUNCTION statement is issued, [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] executes the statement. However, [!INCLUDE[ssNoVersion](../../Topics/TopicNameContainA/tokens/ssNoVersion_md.md)] does not execute the function when it is invoked.  
