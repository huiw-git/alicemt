...
---
# System Variables
  [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] provides a set of system variables that store information about the running package and its objects. These variables can be used in expressions and property expressions to customize packages, containers, tasks, and event handlers.  
...
## System Variables for Packages  
 The following table describes the system variables that [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] provides for packages.  
...
|**InteractiveMode**|Boolean|Indicates whether the package is run in interactive mode. If a package is running in [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer, this property is set to **True**. If a package is running using the **DTExec** command prompt utility, the property is set to **False**.|  
...
|**ServerExecutionID**|Int64|Execution ID for the package that is executed on the [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] server.<br /><br /> The default value is zero. The value is changed only if the package is executed by ISServerExec on the [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] Server. When there is a child package, the value is passed from the parent package to child package.|  
...
## System Variables for Containers  
 The following table describes the system variables that [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] provides for the For Loop, Foreach Loop, and Sequence containers.  
...
## System Variables for Tasks  
 The following table describes the system variables that [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] provides for tasks.  
...
## System Variables for Event Handlers  
 The following table describes the system variables that [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] provides for event handlers. Not all variables are available to all event handlers.  
