---
title: Hash Indexes for Memory-Optimized Tables
H1: na
ms.custom: na
...
- [Indexes for Memory-Optimized Tables](Indexes%20for%20Memory%2dOptimized%20Tables.xml)  
  ...
  - You can instead declare the hash index in a separate [ALTER TABLE...ADD INDEX](#h3-b2-declaration-limitations) statement.  
    ...
    (  
      ...
      INDEX ix_hash_SupportEngineerName  
        HASH (SupportEngineerName) WITH (BUCKET_COUNT = 64)  
    )  
      WITH (  
        MEMORY_OPTIMIZED = ON,  
...
- NONCLUSTERED, which is the default and cannot be overridden. No hash index can be declared as Clustered, because the hash approach has no concept of sequence.  
...
![hekaton_tables_23d](../../Images/Image/ImageNotContaina/hekaton_tables_23d.png "Index)  
...
- Chains within buckets:  
  ...
  - An average chain length of 1 is an ideal, and might be achievable for a UNIQUE hash index.  
  
  
You must understand the data offered by this DMV before you can confidently design and manage a large hash index.  
...
*Primary key index (PK\_SalesOrd\_...):*  
