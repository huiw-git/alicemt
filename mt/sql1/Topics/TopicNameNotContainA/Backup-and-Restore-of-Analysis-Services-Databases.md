...
##  <a name="bkmk_prereq"></a> Prerequisites  
 ...
 Restore location must be an Analysis Services instance that is the same version, or a newer version, as the instance from which the backup was taken. Although you cannot restore a database from a [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] instance to an earlier version of Analysis Services, it is common practice to restore an older version database, such as SQL Server 2012, on a newer [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] instance.  
...
##  <a name="bkmk_cube"></a> Backing Up a Multidimensional or a Tabular Database  
 Administrators can back up an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database to a single [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] backup file \(.abf\), regardless of size of the database. For step by step instructions, see [How to Backup an Analysis Services Database \(TechMantra\)](http://www.mytechmantra.com/LearnSQLServer/Backup_an_Analysis_Services_Database.html) and [Automate Backup an Analysis Services Database \(TechMantra\)](http://www.mytechmantra.com/LearnSQLServer/Automate_Backup_of_Analysis_Services_Database.html).  
  
> [!NOTE]  
>  [!INCLUDE[ssGeminiShort](../../Token/Other/ssGeminiShort_md.md)], used for loading and querying [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data models in a SharePoint environment, loads its models from SharePoint content databases. These content databases are relational and run on the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] relational database engine. As such, there is no [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] backup and restore strategy for [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data models. If you have a disaster recovery plan in place for SharePoint content, that plan encompasses the [!INCLUDE[ssGemini](../../Token/Other/ssGemini_md.md)] data models stored in the content databases.  
  
 **Remote Partitions**  
  
 If the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database contains remote partitions, the remote partitions should also be backed up. When you back up a database with remote partitions, all the remote partitions on each remote server are backed up to a single file on each of those remote servers respectively. Therefore, if you want to create those remote backups off their respective host computers, you will have to manually copy those files to the designated storage areas.  
  
 **Contents of a backup file**  
  
 Backing up an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database produces a backup file whose contents vary depending upon the storage mode used by the database objects. This difference in backup content results from the fact that each storage mode actually stores a different set of information within an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database. For example, multidimensional hybrid OLAP \(HOLAP\) partitions and dimensions store aggregations and metadata in the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database, while relational OLAP \(ROLAP\) partitions and dimensions only store metadata in the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database. Because the actual contents of an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database vary based on the storage mode of each partition, the contents of the backup file also vary. The following table associates the contents of the backup file to the storage mode used by the objects.  
...
> [!NOTE]  
>  Backing up an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database does not back up the data in any underlying data sources, such as a relational database. Only the contents of the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database are backed up.  
  
 When you back up an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database, you can choose from the following options:  
...
-   Whether to encrypt the contents of the backup files and require a password before the file can be unencrypted and restored. By default, the backed up data is not encrypted.  
    ...
    > [!IMPORTANT]  
    >  For each backup file, the user who runs the backup command must have permission to write to the backup location specified for each file. Also, the user must have one of the following roles: a member of a server role for the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance, or a member of a database role with Full Control \(Administrator\) permissions on the database to be backed up.  
  
 For more information about backing up an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database, see [Backup Options](../../Topics/TopicNameNotContainA/Backup-Options.md).  
  
##  <a name="bkmk_restore"></a> Restoring an Analysis Services Database  
 Administrators can restore an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database from one or more backup files.  
  
> [!NOTE]  
>  If a backup file is encrypted, you must provide the password specified during backup before you can use that file to restore an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database.  
...
-   You can choose to have the restore command change the restoration folder for each partition being restored. Local partitions can be restored to any folder location that is local to the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance to which the database is being restored. Remote partitions can be restored to any folder on any server, other than the local server; remote partitions cannot become local.  
 ...
 For more information about restoring an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] database, see [Restore Options](../../Topics/TopicNameNotContainA/Restore-Options.md).  
