...
> [!NOTE]  
>  If you use a different data mining service than the ones provided in [!INCLUDE[ssASCurrent](../../Token/Other/ssASCurrent_md.md)], or if you create your own plug-in algorithms, additional custom node types may be available.  
...
##  <a name="bkmk_AlgoType"></a> Mining Model Content by Algorithm Type  
 Each algorithm stores different types of information as part of its content schema. For example, the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Clustering Algorithm generates many child nodes, each of which represents a possible cluster. Each cluster node contains rules that describe characteristics shared by items in the cluster. In contrast, the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] Linear Regression algorithm does not contain any child nodes; instead, the parent node for the model contains the equation that describes the linear relationship discovered by analysis.  
...
##  <a name="bkmk_Querying"></a> Tools for Querying Mining Model Content  
 ...
 The easiest way to create a content query is to execute the following DMX statement in [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)]:  
...
```  
 ...
 In [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)], you can also access the information in the data mining schema rowsets by opening a connection to the [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] instance and querying the system tables. For more information, see [Data Mining Schema Rowsets &#40;SSAs&#41;](../../Topics/TopicNameNotContainA/Data-Mining-Schema-Rowsets--SSAs-.md).  
