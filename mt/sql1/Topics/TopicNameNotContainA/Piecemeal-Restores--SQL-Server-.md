...
# Piecemeal Restores (SQL Server)
  This topic is relevant only for databases in the Enterprise edition of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] that contain multiple files or filegroups; and, under the simple model, only for read\-only filegroups.  
...
## Piecemeal Restore Scenarios  
 All editions of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] support offline piecemeal restores. In the Enterprise edition, a piecemeal restore can be either online or offline. The implications of offline and online piecemeal restores are as follows:  
...
-   [!INCLUDE[hek_1](../../Token/Other/hek_1_md.md)] piecemeal restore scenario  
...
-   Initial stage \(restore and recover the primary filegroup and all read\/write filegroups\)  
    ...
    -   Specify the READ\_WRITE\_FILEGROUPS option if the backup is a full database backup.  
...
1.  Create a partial backup \(backup\_1\) of a simple\-model database.  
...
3.  Later, a file restore of a read\/write secondary filegroup WITH NORECOVERY from the backup\_1 partial backup  
