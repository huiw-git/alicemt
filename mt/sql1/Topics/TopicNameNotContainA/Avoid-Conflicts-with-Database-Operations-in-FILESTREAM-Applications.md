...
# Avoid Conflicts with Database Operations in FILESTREAM Applications
  Applications that use SqlOpenFilestream\(\) to open Win32 file handles for reading or writing FILESTREAM BLOB data can encounter conflict errors with [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements that are managed in a common transaction. This includes [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] or MARS queries that take a long time to finish execution. Applications must be carefully designed to help avoid these types of conflicts.  
  
 When [!INCLUDE[ssDEnoversion](../../Token/Other/ssDEnoversion_md.md)] or applications try to open FILESTREAM BLOBs, the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] checks the associated transaction context. The [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] allows or denies the request based on whether the open operation is working with DDL statements, DML statements, retrieving data, or managing transactions. The following table shows how the [!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)] determines whether a [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statement will be allowed or denied based on the type of files that are open in the transaction.  
...
## Examples  
 The following examples show how [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements and FILESTREAM Win32 access can cause conflicts.  
