...
# Specify a Column to Use as Regressor in a Model
 ...
 However, you can specify that a column be included as a regressor by adding the FORCE\_REGRESSOR parameter to the model and specifying the regressors to use. You might want to do this in cases where the attribute has meaning even if the effect is too small to be detected by the model, or when you want to ensure that the attribute is included in the formula.  
...
7.  For the parameter, FORCE\_REGRESSOR, in the **Value** column, type the column names enclosed in brackets and separated by a comma, as follows:  
