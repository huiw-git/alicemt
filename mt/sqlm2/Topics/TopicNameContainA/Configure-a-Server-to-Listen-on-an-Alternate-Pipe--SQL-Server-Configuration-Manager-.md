...
# Configure a Server to Listen on an Alternate Pipe (SQL Server Configuration Manager)
  This topic describes how to configure a server to listen on an alternate pipe in [!INCLUDE[ssCurrent](../../Token\Other/ssCurrent_md.md)] by using SQL Server Configuration Manager. By default, the default instance of [!INCLUDE[ssDEnoversion](../../Token\Other/ssDEnoversion_md.md)] listens on named pipe \\\\.\\pipe\\sql\\query. Named instances of [!INCLUDE[ssDEnoversion](../../Token\Other/ssDEnoversion_md.md)] and [!INCLUDE[ssEW](../../Token\Other/ssEW_md.md)] listen on other pipes.  
  
 There are three ways to connect to a specific named pipe with a client application:  
  
-   Run the [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)] Browser service on the server.  
...
3.  On the **Protocol** tab, in the **Pipe Name** box, type the pipe you want the [!INCLUDE[ssDE](../../Token\Other/ssDE_md.md)] to listen on, and then click **OK**.  
  
4.  In the console pane, click **SQL Server Services**.  
  
5.  In the details pane, right\-click **SQL Server \(**\<instance name\>**\)** and then click **Restart**, to stop and restart [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)].  
  
 When [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)] is listening on an alternate pipe, there are three ways to connect to a specific named pipe with a client application:  
