...
3.  Connect to the principal server and issue the following statement:  
     ...
     ALTER DATABASE *\<database\_name\>* SET WITNESS **\=***\<server\_network\_address\>*  
  
     where *\<database\_name\>* is the name of the database to be mirrored \(this name is the same on both partners\), and *\<server\_network\_address\>* is the server network address of the witness server instance.  
