...
# Preprocess a Schema to Merge Included Schemas
  The W3C XSD **include** element provides support for schema modularity in which an XML schema can be partitioned into more than one physical file. [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)] currently does not support this element. XML schemas that include this element will be rejected by the server.  
