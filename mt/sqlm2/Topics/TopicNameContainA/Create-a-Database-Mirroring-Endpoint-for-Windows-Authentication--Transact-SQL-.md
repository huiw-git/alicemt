...
# Create a Database Mirroring Endpoint for Windows Authentication (Transact-SQL)
  This topic describes how to create a database mirroring endpoint that uses Windows Authentication in [!INCLUDE[ssCurrent](../../Token\Other/ssCurrent_md.md)] by using [!INCLUDE[tsql](../../Token\Other/tsql_md.md)]. To support database mirroring or [!INCLUDE[ssHADR](../../Token\Other/ssHADR_md.md)] each instance of [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)] requires a database mirroring endpoint. A server instance can have only one database mirroring endpoint, which has a single port. A database mirroring endpoint can use any port that is available on the local system when the endpoint is created. All database mirroring sessions on a server instance listen on that port, and all incoming connections for database mirroring use that port.  
...
1.  Connect to the instance of [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)] on which you want to create a database mirroring endpoint.  
...
4.  To use Transact\-SQL to create an endpoint to use with Windows Authentication, use a CREATE ENDPOINT statement. The statement takes the following general form:  
     ...
     AS TCP \( LISTENER\_PORT \= *\<listenerPortList\>* \)  
  
     FOR DATABASE\_MIRRORING  
    ...
    -   *\<role\>* defines the role or roles that the server can perform. Specifying ROLE is required. However, the role of the endpoint is relevant only for database mirroring. For [!INCLUDE[ssHADR](../../Token\Other/ssHADR_md.md)], the role of the endpoint is ignored.  
        ...
        > [!NOTE]  
        >  For more information about Database Mirroring options for different editions of [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)], see [Features Supported by the Editions of SQL Server 2016](../../Topics\TopicNameNotContainA/Features-Supported-by-the-Editions-of-SQL-Server-2016.md).  
