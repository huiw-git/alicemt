...
> [!NOTE]  
> The [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent service account must be part of the Pre-Windows 2000 Compatible Access group on the domain controller, or jobs that are owned by domain users who are not members of the Windows Administrators group will fail.  
  
-   In Windows servers, the account that the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent Service runs as requires the following permissions to be able to support [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent proxies.  
...
> [!NOTE]  
> To receive WMI alert notification, the service account for [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent must have been granted permission to the namespace that contains the WMI events, and ALTER ANY EVENT NOTIFICATION.  
  
## SQL Server Role Membership  
The account that the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent service runs as must be a member of the following [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] roles:  
...
The following table lists the Windows account types that can be used for the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent service.  
  
|Service account type|Non-clustered Server|Clustered server|Domain controller (non-clustered)|  
|------------------------|-------------------------|--------------------|--------------------------------------|  
|[!INCLUDE[msCoName](../content/includes/msCoName_md.md)] Windows domain account (member of Windows Administrators group)|Supported|Supported|Supported|  
...
[!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent may fail to start if you run the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent service under the Network Service account, and the Network Service account has been explicitly granted access to log into a [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] instance as a [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] user.  
  
To resolve this, reboot the computer where [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] is running. This only needs to be done once.  
  
### Limitation 4: Using the Network Service Account When SQL Server Reporting Services Is Running on the Same Computer  
[!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent may fail to start if you run the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent service under the Network Service account and [!INCLUDE[ssRSnoversion](../content/includes/ssRSnoversion_md.md)] is also running on the same computer.  
  
To resolve this, reboot the computer where [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] is running, and then restart both the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] and the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent services. This only needs to be done once.  
...
> [!NOTE]  
> Use [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Configuration Manager to specify that [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent must start up when the operating system starts.  
