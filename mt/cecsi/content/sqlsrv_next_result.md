...
## Example  
The following example creates and executes a stored procedure that inserts a product review into the *Production.ProductReview* table, and then selects all reviews for the specified product. After execution of the stored procedure, the first result \(the number of rows affected by the INSERT query in the stored procedure\) is consumed without calling **sqlsrv\_next\_result**. The next result \(the rows returned by the SELECT query in the stored procedure\) is made available by calling **sqlsrv\_next\_result** and consumed using [sqlsrv_fetch_array](../content/sqlsrv_fetch_array.md).  
...
When executing a stored procedure that has output parameters, it is recommended that all other results are consumed before accessing the values of output parameters. For more information see [How to: Specify Parameter Direction Using the SQLSRV Driver](../Topic/How%20to:%20Specify%20Parameter%20Direction%20Using%20the%20SQLSRV%20Driver.md).  
  
## Example  
The following example executes a batch query that retrieves product review information for a specified product ID, inserts a review for the product, then again retrieves the product review information for the specified product ID. The newly inserted product review will be included in the final result set of the batch query. The example uses [sqlsrv\_next\_result](../content/sqlsrv_next_result.md) to move from one result of the batch query to the next.  
