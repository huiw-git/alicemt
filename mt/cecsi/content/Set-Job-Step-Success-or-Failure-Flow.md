...
When creating [!INCLUDE[msCoName](../content/includes/msCoName_md.md)] [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent jobs, you can specify what action [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] should take if a failure occurs during job execution. Determine the action that [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] should take upon the success or failure of each job step. Then use the following procedure to configure the job step action flow logic by using [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent.  
