...
|-|-|-|
SQL_COPT_SS_SQLAUTHENTICATION|SQLAuthentication|Must be set before connecting. Connection string value overrides DSN and sets this attribute if also specified. <br><br> Valid values are the same as SQLAuthentication keyword.|
|SQL_COPT_SS_ACCESS_TOKEN|N/A|Accepts an AAD access token. Causes, UID, PWD, Trusted_Connection, and SQLAuthentication connection string keywords and their equivalent attributes if any to be ignored.|
|SQL_COPT_SS_ENCRYPT|Encrypt (yes/no)||	
|SQL_COPT_SS_TRUST_SERVER_CERTIFICATE|TrustServerCertificate (yes/no)||	
|SQL_COPT_SS_OLDPWD|N/A|	Deprecated: Not supported with FedAuth. <br><br>Password expiration for SQL Server Authentication was introduced in SQL Server 2005. The SQL_COPT_SS_OLDPWD attribute was added to allow the client to provide both the old and the new password for the connection. When this property is set, the provider will not use the connection pool for the first connection or for subsequent connections, since the connection string will contain the "old password" which has now changed.|
|SQL_COPT_SS_INTEGRATED_SECURITY|Trusted_Connection (yes/no)|Deprecated: Use SQL_COPT_SS_SQLAUTHENTICATION set to Active Directory Integrated instead. <br><br>Forces use of Windows Authentication for access validation on server login. When Windows Authentication is used, the driver ignores user identifier and password values provided as part of SQLConnect, SQLDriverConnect, or SQLBrowseConnect processing.|


### Example connection strings
1.	SQL Authentication – current (being deprecated) syntax. Server certificate is not validated, and encryption is used only if the server enforces it. The username/password is passed in the connection string.
...
3.	Integrated Windows Authentication using SSPI (to SQL Server or SQL IaaS) – current syntax (being deprecated). Server certificate is not validated, unless encryption is used. 
