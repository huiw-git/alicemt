...
4.  Execute the query with [sqlsrv\_query](../content/sqlsrv_query.md) or with [sqlsrv\_prepare](../content/sqlsrv_prepare.md) and [sqlsrv\_execute](../content/sqlsrv_execute.md). For example, the following code uses the connection *$conn* to execute the query *$tsql* with parameter values specified in *$params*:  
