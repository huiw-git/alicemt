...
> [!NOTE]  
> These properties, accessed using Server Explorer, are read\-only. To edit column properties for [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] databases, select the column in Table Designer. Those properties are described later in this topic.  
...
**Identity Increment**  
Shows the increment that will be added to the **Identity Seed** for each subsequent row of the identity column. (Applies only to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)].)  
  
**Identity Seed**  
Shows the seed value assigned to the first row in the table for the identity column. (Applies only to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)].)  
  
**Is Identity**  
Shows whether the selected column is the identity column for the table. (Applies only to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)].)  
...
> [!NOTE]  
> These properties apply only to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)].  
...
**Is RowGuid**  
Indicates whether [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] uses the column as a ROWGUID. You can set this value to **Yes** only for a column with the data type of **uniqueidentifier**. To edit this property, click its value, expand the drop\-down list, and choose another value.  
...
> [!NOTE]  
> The length of a **varchar(max)** data type varies for each row. sp\_help returns (\-1) as the length of **varchar(max)** column. [!INCLUDE[ssManStudio](../content/includes/ssManStudio_md.md)] displays \-1 as the column size.  
