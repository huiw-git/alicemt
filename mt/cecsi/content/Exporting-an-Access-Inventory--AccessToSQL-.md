...
For more information about connecting to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)], see [Connecting to SQL Server &#40;AccessToSQL&#41;](../content/Connecting-to-SQL-Server--AccessToSQL-.md).  
...
After you export metadata about Access databases, you can query the metadata. The following instructions describe to use the Query Editor window in [!INCLUDE[ssManStudioFull](../content/includes/ssManStudioFull_md.md)] to run queries.  
  
**To query metadata**  
  
1.  From the **Start** menu, point to **All Programs**, point to **Microsoft [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] 2005** or to **Microsoft [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] 2008** or to **Microsoft [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] 2012**, and then click **SQL Server Management Studio**.  
...
## Query Examples  
Before you run any of the following queries, you should run a USE *database\_name* query to make sure the queries are run against the database that contains the exported metadata. For example, if you exported metadata to a database named MyAccessMetadata, you would add the following at the beginning of the [!INCLUDE[tsql](../content/includes/tsql_md.md)] code:  
...
### Which databases contain private information?  
Your Access databases might contain sensitive or personal information. You might want to move these databases to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] to take advantage of its security features. If you know that columns containing sensitive data have a specific name, or contain specific characters, you can use a query to find all columns that contain that information. For example, you can find all columns that include the string "salary".  The query then returns the database name, table name, and column name.  
