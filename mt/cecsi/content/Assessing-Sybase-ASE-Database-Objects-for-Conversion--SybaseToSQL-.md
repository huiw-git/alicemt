...
Before you load objects and migrate data to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure, you should determine how complex the migration will be and how much time the migration will take. SSMA can create an assessment report that shows the percentage of objects and procedures that will be successfully converted to [!INCLUDE[tsql](../content/includes/tsql_md.md)]. SSMA also lets you view the specific issues that will cause conversion failures.  
  
## Creating Assessment Reports  
When it creates this assessment report, SSMA converts the selected Sybase Adaptive Server Enterprise (ASE) database objects to [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure syntax, and then shows the results.  
...
3.  Right\-click **Databases**, and then select **Create Report**.  
    ...
    When the assessment is complete, the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Migration Assistant for Sybase : Assessment Report window will appear.  
...
4.  Review any error messages, and then determine what you want to do with the object that caused the conversion problem:  
    ...
    -   You can exclude the object from migration. In [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure Metadata Explorer and Sybase Metadata Explorer, clear the check box next to the item before you load the objects into [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure and migrate data from ASE.  
