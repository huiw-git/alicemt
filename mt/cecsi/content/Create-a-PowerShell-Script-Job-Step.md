...
This topic describes how to create and define a [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent job step that executes a PowerShell script in [!INCLUDE[ssCurrent](../content/includes/ssCurrent_md.md)] by using [!INCLUDE[ssManStudioFull](../content/includes/ssManStudioFull_md.md)] or [!INCLUDE[tsql](../content/includes/tsql_md.md)].  
...
8.  Click the **Advanced** page to set the following job step options: what action to take if the job step succeeds or fails, how many times [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent should try to execute the job step, and how often retry attempts should be made.  
