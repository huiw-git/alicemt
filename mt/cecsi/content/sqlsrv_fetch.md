...
---
# sqlsrv_fetch
Makes the next row of a result set available for reading. Use [sqlsrv_get_field](../content/sqlsrv_get_field.md) to read fields of the row.  
...
## Example  
The following example uses **sqlsrv_fetch** to retrieve a row of data containing a product review and the name of the reviewer. To retrieve data from the result set, [sqlsrv_get_field](../content/sqlsrv_get_field.md) is used. The example assumes that SQL Server and the [AdventureWorks](http://go.microsoft.com/fwlink/?LinkID=67739) database are installed on the local computer. All output is written to the console when the example is run from the command line.  
