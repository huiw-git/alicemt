...
Note that in the first call to a **sqlsrv** function \([sqlsrv\_configure](../content/sqlsrv_configure.md)\), warnings are treated as errors. Because warnings are added to the error collection, you do not have to check for warnings separately from errors. In subsequent calls to **sqlsrv** functions, however, warnings will not be treated as errors, so you must check explicitly for warnings and for errors.  
...
## See Also  
[How to: Configure Error and Warning Handling Using the SQLSRV Driver](../Topic/How%20to:%20Configure%20Error%20and%20Warning%20Handling%20Using%20the%20SQLSRV%20Driver.md)  
