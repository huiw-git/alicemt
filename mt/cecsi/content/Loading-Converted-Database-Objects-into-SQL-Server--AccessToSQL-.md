...
If you want to load the converted database objects into [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure without modification, you can have SSMA directly create or recreate the database objects. That method is quick and easy, but does not allow for customization of the [!INCLUDE[tsql](../content/includes/tsql_md.md)] code that defines the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure objects, other than stored procedures.  
...
> [!NOTE]  
> You can select existing [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure database objects that were not converted from Access databases. However, SSMA will not re\-create or alter those objects.  
...
1.  In [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Metadata Explorer, expand the top node (the server name) and then expand **Databases**.  
...
### Modifying Scripts  
After you have saved the [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] or SQL Azure object definitions as a script, you can use [!INCLUDE[ssManStudioFull](../content/includes/ssManStudioFull_md.md)] to modify the script.  
  
**To modify a script**  
  
1.  On the [!INCLUDE[ssManStudio](../content/includes/ssManStudio_md.md)] **File** menu, point to **Open**, and then click **File**.  
