...
1.  The default recovery model of the [!INCLUDE[ssSampleDBobject](../../Token/Other/ssSampleDBobject_md.md)] database is the simple recovery model. Because this recovery model does not support restoring to the point of a failure, set [!INCLUDE[ssSampleDBobject](../../Token/Other/ssSampleDBobject_md.md)] to use the full recovery model by running the following [ALTER DATABASE](../Topic/ALTER%20DATABASE%20\(Transact-SQL\).md) statement:  
...
3.  Create a routine log backup:  
 ...
 The following example restores the backups that are created previously, after creating a tail\-log backup of the [!INCLUDE[ssSampleDBobject](../../Token/Other/ssSampleDBobject_md.md)] database. \(This step assumes that the log disk can be accessed.\)  
