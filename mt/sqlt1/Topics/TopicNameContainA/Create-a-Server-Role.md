...
# Create a Server Role
  This topic describes how to create a new server role in [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] by using [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] or [!INCLUDE[tsql](../../Token/Other/tsql_md.md)].  
...
-   Also requires IMPERSONATE on the *server\_principal* for logins, ALTER permission for server roles used as the *server\_principal*, or membership in a Windows group that is used as the server\_principal.  
...
4.  In the **New Server Role –***server\_role\_name* dialog box, on the **General** page, enter a name for the new server role in the **Server role name** box.  
