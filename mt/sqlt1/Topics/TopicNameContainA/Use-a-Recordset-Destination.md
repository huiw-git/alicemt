...
1.  In [!INCLUDE[ssBIDevStudioFull](../../Token/Other/ssBIDevStudioFull_md.md)], create or open an [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] package.  
...
5.  Add a Data Flow task to the package, and on the Data Flow tab of [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer, configure sources and transformations to load and transform the data.  
...
7.  On the Control Flow tab of [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer, add a Foreach Loop container and connect this container after the Data Flow task. Then, open the **Foreach Loop Editor** to configure the container with the following settings:  
...
## Example of Using the Recordset Destination  
 In the following example, the Data Flow task loads information about [!INCLUDE[ssSampleDBnormal](../../Token/Other/ssSampleDBnormal_md.md)] employees from the Sales.SalesPerson table into a Recordset destination. Then, a Foreach Loop container reads one row of data at a time, and calls a Send Mail task. The Send Mail task uses expressions to send a customized e\-mail message to each salesperson about the amount of his or her bonus.  
  
#### To create the project and configure the variables  
  
1.  In [!INCLUDE[ssBIDevStudio](../../Token/Other/ssBIDevStudio_md.md)], create a new [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] project.  
...
1.  In the Connection Managers area of the [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer, add and configure a new OLE DB connection manager that connects to the [!INCLUDE[ssSampleDBnormal](../../Token/Other/ssSampleDBnormal_md.md)] sample database.  
...
1.  On the **Control Flow** tab of [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer, add a Data Flow task to the design surface.  
...
1.  On the **Control Flow** tab of [!INCLUDE[ssIS](../../Token/Other/ssIS_md.md)] Designer, add a Foreach Loop container, and connect the container after the Data Flow task.  
...
4.  Open the **Send Mail Task Editor**, and then on the **Mail** page, configure the task with the following settings:  
    ...
    2.  For **From**, enter an appropriate e\-mail address.  
         ...
         If you use your own e\-mail address, you will be able to confirm that the package runs successfully. You will receive undeliverable receipts for the messages sent by the Send Mail task to the fictitious salespersons of [!INCLUDE[ssSampleDBnormal](../../Token/Other/ssSampleDBnormal_md.md)].  
...
7.  Run the package.  
     ...
     If you have specified a valid SMTP server and provided your own e\-mail address, you will receive undeliverable receipts for the messages that the Send Mail task sends to the fictitious salespersons of [!INCLUDE[ssSampleDBnormal](../../Token/Other/ssSampleDBnormal_md.md)].  
