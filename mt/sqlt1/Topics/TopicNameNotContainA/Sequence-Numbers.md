...
## Limitations  
 ...
 If there are multiple instances of the **NEXT VALUE FOR** function specifying the same sequence generator within a single [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statement, all those instances return the same value for a given row processed by that [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statement. This behavior is consistent with the ANSI standard.  
...
## Examples  
 There are additional examples in the topics [CREATE SEQUENCE &#40;Transact-SQL&#41;](../Topic/CREATE%20SEQUENCE%20\(Transact-SQL\).md), [NEXT VALUE FOR &#40;Transact-SQL&#41;](../Topic/NEXT%20VALUE%20FOR%20\(Transact-SQL\).md), and [sp\_sequence\_get\_range](../Topic/sp_sequence_get_range%20\(Transact-SQL\).md).  
...
```  
 ...
 [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statements that use `SELECT *` will receive the new column as the last column instead of the first column. If this is not acceptable, then you must create an entirely new table, move the data to it, and then recreate the permissions on the new table.  
