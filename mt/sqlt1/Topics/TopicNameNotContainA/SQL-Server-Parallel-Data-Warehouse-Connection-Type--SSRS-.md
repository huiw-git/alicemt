...
# SQL Server Parallel Data Warehouse Connection Type (SSRS)
  [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[ssDWCurrentFull](../../Token/Other/ssDWCurrentFull_md.md)] is a scalable data warehouse appliance that delivers performance and scalability through massively parallel processing. [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] uses [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] databases for distributed processing and data storage.  
  
 The appliance partitions large database tables across multiple physical nodes, with each node running its own instance of [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)]. When a report connects to [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] to retrieve report data, it connects to the control node, which manages query processing, in the [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] appliance. After the connection is made, there are no differences between working with an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] that is and is not within a [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] environment.  
  
 To include data from [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] in your report, you must have a dataset that is based on a report data source of type [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Parallel Data Warehouse. This built\-in data source type is based on the [!INCLUDE[msCoName](../../Token/Other/msCoName_md.md)] [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Parallel Data Warehouse data extension. Use this data source type to connect to and retrieve data from [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)].  
...
##  <a name="Credentials"></a> Credentials  
 [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] provides its own security technology to implement and store user names and passwords. You cannot use Windows Authentication. If you attempt to connect to [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] using Windows Authentication an error occurs.  
...
-   Type or paste a query. Use the text\-based query designer to enter [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] text directly, to paste query text from another source, to enter complex queries that cannot be built by using the relational query designer, or to enter query\-based expressions.  
...
-   Import an existing query from a file or report. Use the **Import** query button from either query designer to browse to a .sql file or .rdl file and import a query.  
 ...
 The text\-based query designer supports the [Text](#QueryText) mode in which you type [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] commands that select data from the data source.  
  
-   [Text](#QueryText)  
  
 You use [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] with [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] and [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] with [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)]. The two dialects of the SQL language are very similar. Queries written for the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] data source connection type can typically be used for the [!INCLUDE[ssDWCurrentFull](../../Token/Other/ssDWCurrentFull_md.md)] data source connection type.  
  
 A query that retrieves report data from a large database, including a data warehouse such as [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)], might generate a result set that has a very large number of rows unless you aggregate and summarize data to reduce the number of rows that the query returns. You can write queries that include aggregates and grouping by using either the graphical or text\-based query designer.  
  
 [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] support the clause, keyword, and aggregates that the query designer provides to summarize data.  
  
 The graphical query designer used by [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] provides built\-in support for grouping and aggregates to help you write queries that retrieve only summary data. The [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] language features are: the GROUP BY clause, DISTINCT keyword, and aggregates such as SUM and COUNT. The text\-based query designer provides full support for the [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] language, including grouping and aggregates.  
  
 For more information about [!INCLUDE[tsql](../../Token/Other/tsql_md.md)], see [Transact-SQL Reference &#40;Database Engine&#41;](../Topic/Transact-SQL%20Reference%20\(Database%20Engine\).md)in [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] [Books Online](http://go.microsoft.com/fwlink/?LinkId=141687) on msdn.microsoft.com.  
  
###  <a name="QueryText"></a> Using Query Type Text  
 In the text\-based query designer, you type [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] commands to define the data in a dataset. The queries that you use to retrieve data from [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] are the same as ones you use to retrieve data from instances of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] that are not running within a [!INCLUDE[ssDW](../../Token/Other/ssDW_md.md)] application. For example, the following [!INCLUDE[DWsql](../../Token/Other/DWsql_md.md)] query selects the names of all employees who are marketing assistants:  
