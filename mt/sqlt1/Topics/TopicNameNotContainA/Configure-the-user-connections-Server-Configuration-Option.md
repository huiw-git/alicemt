...
# Configure the user connections Server Configuration Option
  This topic describes how to set the **user connections** server configuration option in [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] by using [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] or [!INCLUDE[tsql](../../Token/Other/tsql_md.md)]. The **user connections** option specifies the maximum number of simultaneous user connections that are allowed on an instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. The actual number of user connections allowed also depends on the version of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] that you are using, and also the limits of your application or applications and hardware. [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] allows a maximum of 32,767 user connections. Because **user connections** is a dynamic \(self\-configuring\) option, [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] adjusts the maximum number of user connections automatically as needed, up to the maximum value allowable. For example, if only 10 users are logged in, 10 user connection objects are allocated. In most cases, you do not have to change the value for this option. The default is 0, which means that the maximum \(32,767\) user connections are allowed.  
  
 To determine the maximum number of user connections that your system allows, you can execute [sp\_configure](../Topic/sp_configure%20\(Transact-SQL\).md) or query the [sys.configuration](../Topic/sys.configurations%20\(Transact-SQL\).md) catalog view.  
...
3.  Under **Connections**, in the **Max number of concurrent connections** box, type or select a value from 0 through 32767 to set the maximum number of users that are allowed to connect simultaneously to the instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)].  
...
3.  Copy and paste the following example into the query window and click **Execute**. This example shows how to use [sp\_configure](../Topic/sp_configure%20\(Transact-SQL\).md) to configure the value of the `user connections` option to `325` users.  
