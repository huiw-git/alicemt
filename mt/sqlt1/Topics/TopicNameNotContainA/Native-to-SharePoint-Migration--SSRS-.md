...
## Use Script to migrate content  
 If the migration tool does not meet your needs, you can manually migrate the report server data. The following is a summary of the steps to complete to migrate report items from a one [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] deployment ot another. The approach supports either Native or SharePoint mode as the source or destination servers.  
...
2.  **[!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] RSS scripts:** Write a Visual Basic script that calls Report Server Web service SOAP methods to copy data between databases. Use the **RS.exe** utility to run the script. Rs.exe is installed with [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)].  
 ...
 The following table summarizes the [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] objects you can migrate with scripts:  
