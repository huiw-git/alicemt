...
# Connection to SQL Server for Delete
  When a login without a database role that includes write permission \(for example the **db\_owner** role\) to the MSXDBCDC database attempts to delete an Oracle CDC instance, the Connect to SQL Server dialog box is displayed.  
  
 In this dialog box you must enter the credentials for a login that has write permission to the MSXDBCDC database, such the **db\_owner** database role to delete the Oracle CDC instance.  
