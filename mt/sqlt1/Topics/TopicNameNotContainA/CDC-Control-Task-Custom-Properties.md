...
|CommandTimeout|integer|This value indicates the timeout \(in seconds\) to use when communicating with the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] database. This value is used where the response time from the database is very slow and the default value \(30 seconds\) is not enough.|  
