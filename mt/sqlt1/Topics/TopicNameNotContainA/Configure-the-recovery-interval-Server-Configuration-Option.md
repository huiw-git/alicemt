...
# Configure the recovery interval Server Configuration Option
  This topic describes how to configure the **recovery interval** server configuration option in [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] by using [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)] or [!INCLUDE[tsql](../../Token/Other/tsql_md.md)]. The **recovery interval** option defines an upper limit on the time recovering a database should take. The [!INCLUDE[ssDEnoversion](../../Token/Other/ssDEnoversion_md.md)] uses the value specified for this option to determine approximately how often [automatic checkpoints](../../Topics/TopicNameNotContainA/Database-Checkpoints--SQL-Server-.md) to issue automatic checkpoints on a given database.  
