...
# Row Count Transformation
 ...
 A [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] [!INCLUDE[ssISnoversion](../../Token/Other/ssISnoversion_md.md)] package can use row counts to update the variables used in scripts, expressions, and property expressions. \(For example, the variable that stores the row count can update the message text in an e\-mail message to include the number of rows.\) The variable that the Row Count transformation uses must already exist, and it must be in the scope of the Data Flow task to which the data flow with the Row Count transformation belongs.  
