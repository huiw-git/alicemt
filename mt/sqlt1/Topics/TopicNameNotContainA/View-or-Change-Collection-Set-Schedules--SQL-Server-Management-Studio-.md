...
# View or Change Collection Set Schedules (SQL Server Management Studio)
  You can view or change collection set schedules by using [!INCLUDE[ssManStudioFull](../../Token/Other/ssManStudioFull_md.md)].  
...
4.  To change the existing upload schedule for the collection set, follow these steps:  
    ...
    3.  Click the row with the schedule that you want. For example, to change the schedule to every 5 minutes, click the row where the schedule name is **CollectorSchedule\_Every\_5min.**  
...
4.  Click the row with the schedule that you want. For example, to change the schedule to every 5 minutes, click the row where the schedule name is **CollectorSchedule\_Every\_5min**.  
 ...
 **Run as**  
 Specifies the account used to run the collection set. By default this is the [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Agent Service Account. If proxy accounts are defined, this list includes the names of the available proxy accounts.  
