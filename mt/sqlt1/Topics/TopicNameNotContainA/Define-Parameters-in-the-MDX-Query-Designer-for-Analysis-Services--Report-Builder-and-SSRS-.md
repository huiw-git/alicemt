...
# Define Parameters in the MDX Query Designer for Analysis Services (Report Builder and SSRS)
  To parameterize an MDX query for an [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] data source, you must add a query parameter to the query. In the MDX query designer, you can add a query parameter in both Design mode and Query mode by specifying a filter. After you define the query with a query parameter, Reporting Services automatically creates a report parameter and a dataset to provide the list of valid values. This enables a user to specify a value that is passed directly to the query.  
...
1.  In the Report Data pane, right\-click on a dataset created from a [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] [!INCLUDE[ssASnoversion](../../Token/Other/ssASnoversion_md.md)] data source type, and then click **Query**. The MDX query designer opens in Design mode.  
...
3.  On the MDX query designer toolbar, click **Query Parameters** \(![Icon for the Query Parameters dialog box](../../Images/Image/ImageNotContaina/IconQueryParameter.gif "IconQueryParameter")\). The Query Parameters dialog box opens.  
