...
|DatabaseID|**int**|The current database ID of the user connection. ID of the database specified by the USE *database* statement or the default database if no USE *database* statement has been issued for a given instance. Determine the value for a database by using the DB\_ID function.|3|Yes|  
