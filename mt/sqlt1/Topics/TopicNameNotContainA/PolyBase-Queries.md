...
---
# PolyBase Queries
  Here are example queries using the [PolyBase Guide](../../Topics/TopicNameNotContainA/PolyBase-Guide.md) feature of SQL Server 2016. Before using these examples, you must also understand the T\-SQL statements required to setup PolyBase \(See [PolyBase T-SQL objects](../../Topics/TopicNameNotContainA/PolyBase-T-SQL-objects.md).\)  
...
### Pushdown for selecting a subset of rows  
 ...
 Here SQL Server 2016 initiates a map\-reduce job to retrieve the rows that match the predicate customer.account\_balance \< 200000 on Hadoop. Since the query can complete successfully without scanning all of the rows in the table, only the rows that meet the predicate criteria are copied to SQL Server. This saves significant time and requires less temporary storage space when the number of customer balances \< 200000 is small in comparison with the number of customers with account balances \>\= 200000.  
  Copy imageCopy Code   
SELECT \* FROM customer WHERE customer.account\_balance \< 200000.  
...
### Pushdown for selecting a subset of columns  
 ...
 In this query, SQL Server initiates a map\-reduce job to pre\-process the Hadoop delimited\-text file so that only the data for the two columns, customer.name and customer.zip\_code, will be copied to SQL Server PDW.  
...
-   Unary operators \(NOT, IS NULL, IS NOT NULL\).  
 ...
 This query has multiple predicates that can be pushed down to Hadoop. SQL Server can push map\-reduce jobs to Hadoop to perform the predicate customer.account\_balance \<\= 200000. The expression BETWEEN 92656 and 92677 is comprised of binary and logical operations that can be pushed to Hadoop. The logical AND of customer.account\_balance and customer.zipcode is a final expression.  
...
## Next steps  
 To learn more about troubleshooting, see [PolyBase troubleshooting](../../Topics/TopicNameNotContainA/PolyBase-troubleshooting.md).  
