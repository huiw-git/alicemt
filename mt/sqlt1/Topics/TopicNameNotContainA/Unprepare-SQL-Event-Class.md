...
# Unprepare SQL Event Class
  The Unprepare SQL event class indicates that the SqlClient, ODBC, OLE DB, or DB\-Library has unprepared \(deleted\) a prepared [!INCLUDE[tsql](../../Token/Other/tsql_md.md)] statement or statements.  
...
|LoginSid|**image**|Security identification number \(SID\) of the logged\-in user. You can find this information in the sys.server\_princiapls catalog. Each SID is unique for each login in the server.|41|Yes|  
