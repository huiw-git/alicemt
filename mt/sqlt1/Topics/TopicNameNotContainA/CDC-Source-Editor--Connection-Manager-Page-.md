...
# CDC Source Editor (Connection Manager Page)
  Use the **Connection Manager** page of the **CDC Source Editor** dialog box to select the ADO.NET connection manager for the [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] database that the CDC source reads change rows from \(the CDC database\). Once the CDC database is selected you need to select a captured table in the database.  
...
-   **Net with update mask**: This mode is similar to the regular Net mode but it also adds boolean columns with the name pattern **\_\_$\<column\-name\>\_\_Changed** that indicate changed columns in the current change row.  
 ...
 **Include reprocessing indicator column**  
 Select this check box to create a special output column called **\_\_$reprocessing**.  
