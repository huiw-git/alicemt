...
# Compute Capacity Limits by Edition of SQL Server
  This topic discusses compute capacity limits for different editions of [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)] and how they differ in physical and virtualized environments with hyperthreaded processors.  
  
 ![Mappings to compute capacity limits](../../Images/Image/ImageNotContaina/Compute_Capacity_limits.gif "Compute_Capacity_limits")  
...
-   A thread or logical processor is one logical computing engine from the perspective of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)], the operating system, an application or driver.  
...
-   But the compute capacity of the 2 logical processors in the hyperthreaded core is greater than the compute capacity of the same core with hyperthreading disabled.  
 ...
 Each edition of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] has two compute capacity limits:  
...
2.  A maximum number of cores as reported by the operating system.  
 ...
 These limits apply to a single instance of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)]. They represent the maximum compute capacity that a single instance will use. They do not constrain the server upon which the instance may be deployed. In fact deploying multiple instances of [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] on the same physical server is an efficient way to use the compute capacity of a physical server with more sockets and\/or cores than the capacity limits below.  
  
 The following table specifies the compute capacity limits for a single instance of each edition of [!INCLUDE[ssCurrent](../../Token/Other/ssCurrent_md.md)]:  
  
|[!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] Edition|Maximum Compute Capacity Used by a Single Instance \([!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)][!INCLUDE[ssDE](../../Token/Other/ssDE_md.md)]\)|Maximum Compute Capacity Used by a Single Instance \(AS, RS\)|  
|---------------------------------------|--------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|  
...
|Express with Advanced Services|Limited to lesser of 1 Socket or 4 cores|Limited to lesser of 1 Socket or 4 cores|  
 ...
 \*Enterprise Edition with Server \+ Client Access License \(CAL\) based licensing \(not available for new agreements\) is limited to a maximum of 20 cores per [!INCLUDE[ssNoVersion](../../Token/Other/ssNoVersion_md.md)] instance. There are no limits under the Core\-based Server Licensing model.  
