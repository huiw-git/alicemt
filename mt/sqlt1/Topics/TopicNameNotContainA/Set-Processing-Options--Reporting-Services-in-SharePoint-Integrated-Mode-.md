...
# Set Processing Options (Reporting Services in SharePoint Integrated Mode)
  You can set processing options on a [!INCLUDE[ssRSnoversion](../../Token/Other/ssRSnoversion_md.md)] report to determine when data processing occurs. You can also set a time\-out value for report processing, and options that determine whether report history is enabled for the current report.  
