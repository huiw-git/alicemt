...
Use the following required steps to create a remote connection profile by using the [!INCLUDE[cm6long](../System Center Configuration Manager/itokens/cm6long_md.md)]**Create Remote Connection Profile Wizard**.  
  
 Before you use this procedure, ensure you have read the introductory information and prerequisites in [Working with remote connection profiles in System Center Configuration Manager](../System Center Configuration Manager/Working-with-remote-connection-profiles-in-System-Center-Configuration-Manager.md).  
...
5.  On the **Profile** settings page of the wizard, specify the following settings for the remote connection profile:  
    ...
    -   **Full name and port of the Remote Desktop Gateway server (optional)** - Specify the name of the Remote Desktop Gateway Server to be used for connections.  
        ...
        > [!NOTE]  
        >  [!INCLUDE[cmshort](../System Center Configuration Manager/itokens/cmshort_md.md)] does not support an internationalized domain name to be used to specify a server in this box.  
        >   
...
4.  In the **Deploy Remote Connection Profile** dialog box, specify the following information:  
    ...
    -   **Allow remediation outside the maintenance window** - If a maintenance window has been configured for the collection to which you deploy the remote connection profile, enable this option to let [!INCLUDE[cmshort](../System Center Configuration Manager/itokens/cmshort_md.md)] remediate the remote connection profile outside the maintenance window. For more information about maintenance windows, see [How to use maintenance windows in System Center Configuration Manager](../System Center Configuration Manager/How-to-use-maintenance-windows-in-System-Center-Configuration-Manager.md).  
    ...
    >   
    >  If two remote connection profiles are deployed to the same device collection, where in one profile **Remediate noncompliant rules when supported** is checked and in the other profile, the same option is not checked, and the two remote connection profiles contain different connection settings, then the profile in which the option is not checked might override the settings in the other profile. This type of remote connection profile deployment is not supported by [!INCLUDE[cmshort](../System Center Configuration Manager/itokens/cmshort_md.md)].  
...
### View compliance results with reports  
 [!INCLUDE[cmshort](../System Center Configuration Manager/itokens/cmshort_md.md)] includes built-in reports that you can use to monitor information about remote connection profiles. These reports have the report category of **Compliance and Settings Management**.  
