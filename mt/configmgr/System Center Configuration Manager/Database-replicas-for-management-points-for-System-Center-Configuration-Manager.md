...
[!INCLUDE[cm6long](../System Center Configuration Manager/itokens/cm6long_md.md)] primary sites can use a database replica to reduce the CPU load placed on the site database  server by management points as they service requests from clients.  
...
-   [Prepare to use database replicas](#bkmk_Prepare)  
  
-   [Configure database replicas](#BKMK_DBReplica_Config)  
  
-   [Manage database replica configurations](#BKMK_DBReplicaOps)  
  
    -   [Uninstall a database replica](#BKMK_UninstallDbReplica)  
...
-   **Upgrades to [!INCLUDE[cm6long](../System Center Configuration Manager/itokens/cm6long_md.md)]**: Before you upgrade a site from [!INCLUDE[cm5short](../System Center Configuration Manager/itokens/cm5short_md.md)] to [!INCLUDE[cm6long](../System Center Configuration Manager/itokens/cm6long_md.md)], you must disable database replicas for management points.  After your site upgrades, you can reconfigure the database replicas for management points.  
...
###  <a name="BKMK_UninstallDbReplica"></a> Uninstall a database replica  
 When you use a database replica for a management point, you might need to uninstall the database replica for a period of time, and then reconfigure it for use. For example, you must remove database replicas before you upgrade a [!INCLUDE[cmshort](../System Center Configuration Manager/itokens/cmshort_md.md)] site to a new service pack. After the site upgrade completes, you can restore the database replica for use.  
  
 Use the following steps to uninstall a database replica.  
  
1.  In the **Administration** workspace of the [!INCLUDE[cmshort](../System Center Configuration Manager/itokens/cmshort_md.md)] console, expand **Site Configuration**, then select **Servers and Site System Roles**, and then in the details pane select the site system server that hosts the management point that uses the database replica you will uninstall.  
